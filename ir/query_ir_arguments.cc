...
std::vector<std::string> InternalSingleQueryIROutputAliases(
    const SingleQueryIR &query) {
  const SingleQueryIR *last = query.Last();
...
class QueryIRArgumentFinalizer {
 public:
  explicit QueryIRArgumentFinalizer(const ast::SemanticTable &semantic_table)
      : semantic_table_(&semantic_table) {}

  void Finalize(QueryIR &query) const { FinalizeQueryIRArguments(query, {}); }
 ...
 private:
  ...
  void FinalizeSingleQueryIRArguments(
    ...
    CHECK(query != nullptr, common::InternalError, "query IR is null");
    SingleQueryIR *segment = query;
    while (segment != nullptr) {
      const std::unordered_set<std::string> dependencies =
          SingleQueryIRDependencies(*segment);
      ...
      std::unordered_set<std::string> output_symbols =
          SingleQueryIROutputSymbols(*segment);
  ...
  [[nodiscard]] std::unordered_set<std::string> SingleQueryIRDependencies(
      const SingleQueryIR &query) const {
...
void FinalizeQueryIRArguments(QueryIR &query,
  ...
  QueryIRArgumentFinalizer finalizer(semantic_table);
...
std::vector<std::string> SingleQueryIROutputAliases(
    const SingleQueryIR &query) {
  return InternalSingleQueryIROutputAliases(query);
}

std::vector<std::string> QueryIROutputAliases(const QueryIR &query) {
  switch (query.Kind()) {
    case QueryIRKind::kSingle:
      return SingleQueryIROutputAliases(query.RequireSingle());
    case QueryIRKind::kUnion: {
      const UnionQueryIR &union_query = query.RequireUnion();
