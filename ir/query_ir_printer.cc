...
std::string NestedIRExpressionKindToString(NestedIRExpressionKind kind) {
  ...
  RG_THROW(common::ErrorCode::InternalError,
           "unknown nested IR expression kind");
}
...
class QueryIRPrinter {
 public:
  explicit QueryIRPrinter(std::ostream &out) : out_(out) {}

  void Print(const QueryIR &query) { PrintQueryIRNode(query); }
 ...
 private:
  ...
  void Indent() { ++indent_; }
  void Dedent() {
    RG_CHECK(indent_ > 0, common::ErrorCode::InternalError,
             "query IR printer indent underflow");
    --indent_;
  }

  void PrintQueryIRNode(const QueryIR &query) {
    switch (query.Kind()) {
  ...
  void PrintUnion(const UnionQueryIR &query) {
    Line("UnionQueryIR");
    Indent();
    ...
    Line("lhs:");
    Indent();
    RG_CHECK(query.lhs != nullptr, common::ErrorCode::InvalidParameter,
  ...
  void PrintSelections(const Selections &selections) {
    ...
    for (const auto &predicate : selections.predicates) {
      ...
      if (predicate.subquery != nullptr) {
        Line("subquery:");
        Indent();
        PrintQueryIRNode(*predicate.subquery);
        Dedent();
      }
