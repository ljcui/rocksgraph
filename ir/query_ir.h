...
enum class QueryIRKind { kSingle, kUnion };

class QueryIR {
 public:
  QueryIR() = default;
  QueryIR(const QueryIR &) = delete;
  QueryIR &operator=(const QueryIR &) = delete;
  QueryIR(QueryIR &&) noexcept = default;
  QueryIR &operator=(QueryIR &&) noexcept = default;
  virtual ~QueryIR() = default;

  [[nodiscard]] virtual QueryIRKind Kind() const = 0;

  [[nodiscard]] const SingleQueryIR &RequireSingle() const;
  SingleQueryIR &RequireSingle();

  [[nodiscard]] const UnionQueryIR &RequireUnion() const;
  UnionQueryIR &RequireUnion();
};

struct SingleQueryIR final : public QueryIR {
  ...
  [[nodiscard]] QueryIRKind Kind() const final { return QueryIRKind::kSingle; }

  [[nodiscard]] const SingleQueryIR *Last() const;
  SingleQueryIR *Last();
};

struct UnionQueryIR final : public QueryIR {
  std::unique_ptr<QueryIR> lhs;
  SingleQueryIR rhs;
  bool all = false;
  bool distinct = true;
  struct UnionMapping {
  ...
  UnionQueryIR() = default;

  [[nodiscard]] QueryIRKind Kind() const final { return QueryIRKind::kUnion; }
};

std::unique_ptr<QueryIR> MakeSingleQueryIR(SingleQueryIR query);
std::unique_ptr<QueryIR> MakeUnionQueryIR(std::unique_ptr<QueryIR> lhs,
                                          SingleQueryIR rhs, bool all);

// CreateQueryIR expects a semantic AST after the default rewriter
