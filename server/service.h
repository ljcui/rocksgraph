...
class Service {
 ...
 public:
   ...
   *
   * \param service_name  Name of the service. This will be used to identify the
   * service. If the service name is already used by another program, Start()
   *                      will issue a warning and exit, and Stop() will
   * actually stop the existing program.
   */
   ...
   *   On Linux, the process will first check if the service is already running.
   * If not, it spawns a child process which in turn calls Run(), and the parent
   * process will exit.
   *
  ...
  int Start() {
    ...
    if (oldpid != NO_PID_FILE && ProcessRunning(oldpid)) {
      LOG_WARN(
          "Service {} already exists at pid {}. You can stop it and start "
          "again.",
          service_name_, oldpid);
    ...
    if (pid > 0) {
      ...
      if (SetServicePid(pid) != 0) {
        LOG_WARN(
            "Failed to write pid file. Make sure you are running as root. ");
        return ACCESS_FAILED;
      }
      return 0;
    } else {
 ...
 private:
  static bool ProcessRunning(int pid) { return kill(pid, 0) == 0; }
};
}  // namespace server
