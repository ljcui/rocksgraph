...
ExprText expressionText(const Expression &expr) {
  if (const auto *node = dynamic_cast<const OrExpression *>(&expr)) {
    ...
    const ExprText left = expressionText(*node->left);
    const ExprText right = expressionText(*node->right);
    const std::string left_text = wrapIfNeeded(left, kOr, false);
    const std::string right_text = wrapIfNeeded(right, kOr, true);
    if (left_text.empty() || right_text.empty()) {
      return {};
    }
    return {left_text + " OR " + right_text, kOr};
  }
  if (const auto *node = dynamic_cast<const XorExpression *>(&expr)) {
    ...
    const ExprText left = expressionText(*node->left);
    const ExprText right = expressionText(*node->right);
    const std::string left_text = wrapIfNeeded(left, kXor, false);
    const std::string right_text = wrapIfNeeded(right, kXor, true);
    if (left_text.empty() || right_text.empty()) {
      return {};
    }
    return {left_text + " XOR " + right_text, kXor};
  }
  if (const auto *node = dynamic_cast<const AndExpression *>(&expr)) {
    ...
    const ExprText left = expressionText(*node->left);
    const ExprText right = expressionText(*node->right);
    const std::string left_text = wrapIfNeeded(left, kAnd, false);
    const std::string right_text = wrapIfNeeded(right, kAnd, true);
    if (left_text.empty() || right_text.empty()) {
      return {};
    }
    return {left_text + " AND " + right_text, kAnd};
  }
  if (const auto *node = dynamic_cast<const ComparisonExpression *>(&expr)) {
    ...
    const ExprText left = expressionText(*node->left);
    const ExprText right = expressionText(*node->right);
    const std::string left_text = wrapIfNeeded(left, kComparison, false);
    const std::string right_text = wrapIfNeeded(right, kComparison, true);
    if (left_text.empty() || right_text.empty()) {
      return {};
    }
  ...
  if (const auto *node = dynamic_cast<const AddExpression *>(&expr)) {
    ...
    const ExprText left = expressionText(*node->left);
    const ExprText right = expressionText(*node->right);
    const std::string left_text = wrapIfNeeded(left, kAdd, false);
    const std::string right_text = wrapIfNeeded(right, kAdd, true);
    if (left_text.empty() || right_text.empty()) {
      return {};
    }
    return {left_text + " + " + right_text, kAdd};
  }
  ...
  if (const auto *node = dynamic_cast<const MultiplyExpression *>(&expr)) {
    ...
    const ExprText left = expressionText(*node->left);
    const ExprText right = expressionText(*node->right);
    const std::string left_text = wrapIfNeeded(left, kMultiply, false);
    const std::string right_text = wrapIfNeeded(right, kMultiply, true);
    if (left_text.empty() || right_text.empty()) {
      return {};
    }
    return {left_text + " * " + right_text, kMultiply};
  }
  ...
  if (const auto *node = dynamic_cast<const PowerExpression *>(&expr)) {
    ...
    const ExprText left = expressionText(*node->left);
    const ExprText right = expressionText(*node->right);
    const std::string left_text = wrapIfNeeded(left, kPower, true);
    const std::string right_text = wrapIfNeeded(right, kPower, false);
    if (left_text.empty() || right_text.empty()) {
      return {};
    }
    return {left_text + " ^ " + right_text, kPower};
  }
  if (const auto *node = dynamic_cast<const NotExpression *>(&expr)) {
    ...
    const std::string operand_text = wrapIfNeeded(operand, kUnary, false);
    if (operand_text.empty()) {
      return {};
    }
    return {"NOT " + operand_text, kUnary};
  }
  ...
  if (const auto *node = dynamic_cast<const ListPredicateExpression *>(&expr)) {
    ...
    const ExprText element = expressionText(*node->element);
    const ExprText list = expressionText(*node->list);
    const std::string element_text = wrapIfNeeded(element, kComparison, false);
    const std::string list_text = wrapIfNeeded(list, kComparison, true);
    if (element_text.empty() || list_text.empty()) {
      return {};
    }
