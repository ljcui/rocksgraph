...
TEST(log, TestFindConflict) {
  ...
  struct Test {
    ...
    uint64_t wconflict;
    Test(std::vector<raftpb::Entry> _ents, uint64_t _wconflict)
        : ents(std::move(_ents)), wconflict(_wconflict) {}
  };
  std::vector<Test> tests = {
      ...
      {{EntryHelper{.index = 1, .term = 4}.Done(),
...
TEST(log, TestFindConflictByTerm) {
  auto ents = [](uint64_t fromIndex,
                 std::vector<uint64_t> terms) -> std::vector<raftpb::Entry> {
  ...
  struct Test {
    std::vector<raftpb::Entry>
        ents;  // ents[0] contains the (index, term) of the snapshot
    uint64_t index;
    uint64_t term;
    uint64_t want;
  };
  ...
  for (auto& tt : tests) {
    ...
    st->ApplySnapshot(
        SnapshotHelper{.index = tt.ents[0].index(), .term = tt.ents[0].term()}
            .Done());
...
TEST(log, TestAppend) {
  ...
  struct Test {
    ...
    uint64_t wunstable;
    Test(std::vector<raftpb::Entry> _ents, uint64_t _windex,
         std::vector<raftpb::Entry> _wents, uint64_t _wunstable)
        : ents(std::move(_ents)),
          windex(_windex),
          wents(std::move(_wents)),
          wunstable(_wunstable) {}
  };
  std::vector<Test> tests = {{
                                 {},
                                 2,
...
TEST(log, TestLogMaybeAppend) {
  ...
  std::vector<Test> tests = {
      // not match: term is different
      {
          ...
          lastindex,
          lastindex,
          {EntryHelper{.index = lastindex + 1, .term = 4}.Done()},
          0,
          false,
          commit,
          false,
      ...
      // not match: index out of bound
      {
          ...
          lastindex + 1,
          lastindex,
          {EntryHelper{.index = lastindex + 2, .term = 4}.Done()},
          0,
          false,
          commit,
          false,
      ...
      },
      {
          ...
          true,
          commit,
          false,  // commit do not decrease
      },
      {
          0, 0, lastindex, {}, 0, true, commit, false,  // commit do not
                                                        // decrease
      },
      {
          lastterm,
      ...
      // match with the entry in the middle
      {
          ...
          lastindex - 1,
          lastindex,
          {EntryHelper{.index = lastindex, .term = 4}.Done()},
          lastindex,
          true,
          lastindex,
          false,
  ...
  for (auto& tt : tests) {
    ...
    try {
      uint64_t glasti;
      bool gappend;
      std::tie(glasti, gappend) =
          raftLog->maybeAppend(tt.index, tt.logTerm, tt.committed, tt.ents);
      ...
      if (gappend && !tt.ents.empty()) {
        ...
        Error err;
        std::tie(gents, err) =
            raftLog->slice(raftLog->lastIndex() - tt.ents.size() + 1,
                           raftLog->lastIndex() + 1, noLimit);
        EXPECT_EQ(err, nullptr);
...
TEST(log, TestCompactionSideEffects) {
  ...
  raftLog->append({EntryHelper{.index = raftLog->lastIndex() + 1,
...
TEST(log, TestNextCommittedEnts) {
  ...
  for (auto& tt : tests) {
    ...
    EXPECT_TRUE(
        VectorEquals(tt.wents, raftLog->nextCommittedEnts(tt.allowUnstable)));
  }
}
...
TEST(log, TestNextUnstableEnts) {
  ...
  for (auto& tt : tests) {
    ...
    auto storage = NewMemoryStorage();
    EXPECT_EQ(storage->Append({previousEnts.begin(),
                               previousEnts.begin() + tt.unstable - 1}),
              nullptr);

    // append unstable entries to raftlog
    auto raftLog = detail::newLog(storage);
    raftLog->append(
        {previousEnts.begin() + tt.unstable - 1, previousEnts.end()});

    auto ents = raftLog->nextUnstableEnts();
    if (!ents.empty()) {
      raftLog->stableTo(ents[ents.size() - 1].index(),
                        ents[ents.size() - 1].term());
    }
    EXPECT_TRUE(VectorEquals(tt.wents, ents));
    EXPECT_EQ(previousEnts[previousEnts.size() - 1].index() + 1,
              raftLog->unstable_.offset_);
  }
}
...
TEST(log, TestStableTo) {
  ...
  for (auto& tt : tests) {
    auto raftLog = detail::newLog(NewMemoryStorage());
    raftLog->append({EntryHelper{.index = 1, .term = 1}.Done(),
                     EntryHelper{.index = 2, .term = 2}.Done()});
...
TEST(log, TestStableToWithSnap) {
  ...
  std::vector<Test> tests = {
      ...
      {snapi + 1,
       snapt,
       {EntryHelper{.index = snapi + 1, .term = snapt}.Done()},
       snapi + 2},
      {snapi,
       snapt,
       {EntryHelper{.index = snapi + 1, .term = snapt}.Done()},
...
TEST(log, TestTermWithUnstableSnapshot) {
  ...
  storage->ApplySnapshot(
      SnapshotHelper{.index = storagesnapi, .term = 1}.Done());
  auto l = detail::newLog(storage);
...
TEST(log, TestSlice) {
  ...
  auto storage = NewMemoryStorage();
  EXPECT_EQ(storage->ApplySnapshot(SnapshotHelper{.index = offset}.Done()),
            nullptr);
  ...
  std::vector<Test> tests = {
      ...
      // Low limit.
      {offset + 1, last, halfe.ByteSizeLong() - 1,
      ...
      // TODO(wangzhiyong)
      //{offset + 1, last, halfe.ByteSizeLong() * 2, entries(offset+1,
      // offset+3)},
      {half - 2, half + 1, halfe.ByteSizeLong() * 2, entries(half - 2, half)},
      {half - 1, half + 1, halfe.ByteSizeLong() * 2,
       entries(half - 1, half + 1)},
      {half, last, halfe.ByteSizeLong() * 2, entries(half, half + 2)},
      // Not enough for three.
      {half - 2, half + 1, halfe.ByteSizeLong() * 3 - 1,
       entries(half - 2, half)},
      // Enough for three.
      {half - 1, half + 2, halfe.ByteSizeLong() * 3,
       entries(half - 1, half + 2)},
  };
  for (auto& tt : tests) {
    std::vector<raftpb::Entry> g;
    Error err;
...
TEST(log, TestScan) {
  ...
  for (auto pageSize :
       std::vector<uint64_t>{0, 1, 10, 100, entrySize, entrySize + 1}) {
    for (auto lo = offset + 1; lo < last; lo++) {
      for (auto hi = lo; hi <= last; hi++) {
        std::vector<raftpb::Entry> got;
        l->scan(lo, hi, pageSize,
                [&](const std::vector<raftpb::Entry>& e) -> Error {
  ...
  int iters = 0;
  EXPECT_EQ(errBreak, l->scan(offset + 1, half, 0,
                              [&](const std::vector<raftpb::Entry>&) -> Error {
                                iters++;
                                if (iters == 2) {
                                  return errBreak;
                                }
  ...
  EXPECT_EQ(l->scan(offset + 1, offset + 11, entrySize * 2,
                    [&](const std::vector<raftpb::Entry>& ents) -> Error {
