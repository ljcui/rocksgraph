...
struct Inflights {
  ...
  // size is being dispatched. Full() must be called prior to Add() to verify
  // that there is room for one more message, and consecutive calls to Add()
  ...
  bool operator==(const Inflights& right) const {
    return start_ == right.start_ && count_ == right.count_ &&
           bytes_ == right.bytes_ && size_ == right.size_ &&
           maxBytes_ == right.maxBytes_ && buffer_ == right.buffer_;
  }
};
