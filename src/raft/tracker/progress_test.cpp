...
TEST(tracker, TestProgressString) {
  ...
  const std::string exp =
      "StateSnapshot match=1 next=2 learner paused pendingSnap=123 inactive "
      "inflight=1[full]";
...
TEST(tracker, TestProgressUpdate) {
  ...
  std::vector<Test> tests = {
      ...
      {prevM + 1, prevM + 1, prevN,
       true},  // increase match, do not decrease next
