...
struct Config {
  ...
  bool operator==(const Config& c) const {
    return voters_ == c.voters_ && autoLeave_ == c.autoLeave_ &&
           learners_ == c.learners_ && learnersNext_ == c.learnersNext_;
  }
  ...
  std::string String() {
    ...
    if (!learners_.empty()) {
      buf.append(format(" learners=%s",
                        quorum::MajorityConfig(learners_).String().c_str()));
    }
    if (!learnersNext_.empty()) {
      buf.append(
          format(" learners_next=%s",
                 quorum::MajorityConfig(learnersNext_).String().c_str()));
    }
    if (autoLeave_) {
...
struct ProgressTracker {
  ...
  bool IsSingleton() {
    return config_.voters_[0].data().size() == 1 &&
           config_.voters_[1].data().size() == 0;
  }

  // Committed returns the largest log index known to be committed based on what
  // the voting members of the group have acknowledged.
  uint64_t Committed() {
    return config_.voters_
        .CommittedIndex(detail::matchAckIndexer(progress_.data()))
        .data();
  }
  ...
  // order.
  void Visit(
      const std::function<void(uint64_t id, std::shared_ptr<Progress>&)>& f) {
  ...
  std::vector<uint64_t> LearnerNodes() {
    ...
    std::vector<uint64_t> nodes{config_.learners_.begin(),
                                config_.learners_.end()};
    std::sort(nodes.begin(), nodes.end(), std::less<uint64_t>());
    return nodes;
  }
  ...
  std::tuple<size_t, size_t, quorum::VoteResult> TallyVotes() {
    ...
    return std::make_tuple(granted, rejected,
                           config_.voters_.GetVoteResult(votes_));
  }
};

// MakeProgressTracker initializes a ProgressTracker.
inline ProgressTracker MakeProgressTracker(size_t maxInflight,
                                           uint64_t maxBytes) {
  ProgressTracker p;
