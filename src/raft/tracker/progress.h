...
struct Progress {
  ...
  bool operator==(const Progress& p) const {
    if (match_ != p.match_ || next_ != p.next_ || state_ != p.state_ ||
        pendingSnapshot_ != p.pendingSnapshot_ ||
        recentActive_ != p.recentActive_ ||
        msgAppFlowPaused_ != p.msgAppFlowPaused_ ||
        isLearner_ != p.isLearner_) {
      return false;
    }
  ...
  // MsgAppFlowPaused, PendingSnapshot, and Inflights.
  ...
  // entries being sent in a MsgApp, with the given total bytes size, appended
  // at and after the given log index.
  Error UpdateOnEntriesSend(int entries, uint64_t bytes, uint64_t nextIndex) {
    switch (state_) {
      case StateType::StateReplicate: {
        ...
        // If this message overflows the in-flights tracker, or it was already
        // full, consider this message being a probe, so that the flow is
        // paused.
        msgAppFlowPaused_ = inflights_->Full();
        break;
      }
      case StateType::StateProbe: {
        // TODO(pavelkalinnikov): this condition captures the previous
        // behaviour, but we should set MsgAppFlowPaused unconditionally for
        // simplicity, because any MsgApp in StateProbe is a probe, not only
        // non-empty ones.
        if (entries > 0) {
      ...
      default:
        return Error(format("sending append in unhandled state %s",
  ...
  std::string String() {
    std::string buf;
    buf.append(
        format("%s match=%d next=%d", ToString(state_).c_str(), match_, next_));
    if (isLearner_) {
      buf.append(" learner");
    }
    if (IsPaused()) {
...
struct ProgressMap {
  ...
  const std::unordered_map<uint64_t, std::shared_ptr<Progress>>& data() const {
    return data_;
  }
  std::unordered_map<uint64_t, std::shared_ptr<Progress>>& data() {
    return data_;
  }
  // TODO(wangzhiyong): return nullptr if not exist ?
  std::shared_ptr<Progress>& operator[](size_t index) {
    return data_.at(index);
  };
  const std::shared_ptr<Progress>& operator[](size_t index) const {
    return data_.at(index);
  };
