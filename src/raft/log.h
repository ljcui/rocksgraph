...
struct RaftLog {
  ...
  std::string String() const {
    return format(
        "committed=%d, applied=%d, applying=%d, unstable.offset=%d, "
        "unstable.offsetInProgress=%d, len(unstable.Entries)=%d",
        committed_, applied_, applying_, unstable_.offset_,
        unstable_.offsetInProgress_, unstable_.entries_.size());
  }
  ...
  std::pair<uint64_t, Error> term(uint64_t i) {
    ...
    // The valid term range is [firstIndex-1, lastIndex]. Even though the entry
    // at firstIndex-1 is compacted away, its term is available for matching
    // purposes when doing log appends.
    if (i + 1 < firstIndex()) {
    ...
    for (auto& ne : ents) {
      if (!matchTerm(ne.index(), ne.term())) {
        if (ne.index() <= lastIndex()) {
          ERAFT_INFO(
              "found conflict at index %d [existing term: %d, conflicting "
              "term: %d]",
  ...
  uint64_t append(std::vector<raftpb::Entry> ents) {
    ...
    if (ents[0].index() - 1 < committed_) {
      ERAFT_FATAL("after(%d) is out of range [committed(%d)]",
                  ents[0].index() - 1, committed_);
    }
  ...
  std::pair<uint64_t, bool> maybeAppend(uint64_t index, uint64_t logTerm,
    ...
    if (ci == 0) {
      ...
      ERAFT_FATAL("entry %d conflict with committed entry [committed(%d)]", ci,
                  committed_);
    } else {
      auto offset = index + 1;
      if (ci - offset > ents.size()) {
        ERAFT_FATAL("index, %d, is out of range [%d]", ci - offset,
                    ents.size());
      }
  ...
  // another log, given that the only information known about the other log is
  // the (index, term) of its single entry.
  //
  // Specifically, the first returned value is the max guessIndex <= index, such
  // that term(guessIndex) <= term or term(guessIndex) is not known (because
  // this index is compacted or not yet stored).
  //
  // The second returned value is the term(guessIndex), or 0 if it is unknown.
  //
  // This function is used by a follower and leader to resolve log conflicts
  // after an unsuccessful append to a follower, and ultimately restore the
  // steady flow of appends.
  std::pair<uint64_t, uint64_t> findConflictByTerm(uint64_t index,
                                                   uint64_t _term) {
    for (; index > 0; index--) {
  ...
  // hasNextUnstableEnts returns if there are any entries that are available to
  // be written to the local stable log and are not already in-progress.
  bool hasNextUnstableEnts() { return nextUnstableEnts().size() > 0; }
  ...
  // hasNextOrInProgressSnapshot returns if there is pending snapshot waiting
  // for applying or in the process of being applied.
  ...
  // given callback. The callback can be invoked multiple times, with
  // consecutive sub-ranges of the requested range. Returns up to pageSize bytes
  // worth of entries at a time. May return more if a single entry size exceeds
  // the limit.
  //
  ...
  Error scan(uint64_t lo, uint64_t hi, int pageSize,
             const std::function<Error(const std::vector<raftpb::Entry>&)>& v) {
    while (lo < hi) {
      std::vector<raftpb::Entry> ents;
      Error err;
  ...
  std::pair<std::vector<raftpb::Entry>, Error> slice(uint64_t lo, uint64_t hi,
                                                     uint64_t maxSize) {
    ...
    if (err == ErrCompacted) {
      ...
      ERAFT_FATAL("%s", err.String().c_str());  // TODO(pavelkalinnikov): handle
                                                // errors uniformly
    }
    ...
    // returned slice is shorter than requested (which means the next entry
    // would bring it over the limit), or a single entry reaches the limit.
    if (ents.size() < cut - lo) {
      return {std::move(ents), nullptr};
    }
    // Slow path computes the actual total size, so that unstable entries are
    // cut optimally before being copied to ents slice.
    ...
    // Total size of unstable may exceed maxSize-size only if len(unstable)
    // == 1. If this happens, ignore this extra entry.
  ...
  // appended them to the local raft log yet. If allowUnstable is true,
  // committed entries from the unstable log may be returned; otherwise, only
  // entries known to reside locally on stable storage will be returned.
  std::vector<raftpb::Entry> nextCommittedEnts(bool allowUnstable) {
    ...
    if (err != nullptr) {
      ERAFT_FATAL("unexpected error when getting unapplied entries (%s)",
                  err.String().c_str());
    }
    return ents;
  }

  // hasNextCommittedEnts returns if there is any available entries for
  // execution. This is a fast check without heavy raftLog.slice() in
  // nextCommittedEnts().
  ...
  // be applied to the local storage and is not already in-progress.
  bool hasNextUnstableSnapshot() { return unstable_.nextSnapshot() != nullptr; }
  ...
  void acceptApplying(uint64_t i, uint64_t size, bool allowUnstable) {
    if (committed_ < i) {
      ERAFT_FATAL(
          "applying(%d) is out of range [prevApplying(%d), committed(%d)]", i,
          applying_, committed_);
    }
    applying_ = i;
    applyingEntsSize_ += size;
  ...
  // to be marked as being in-progress, to avoid returning them with future
  // calls to Ready().
  void acceptUnstable() { unstable_.acceptInProgress(); }
  ...
  uint64_t lastTerm() {
    ...
    if (err != nullptr) {
      ERAFT_FATAL("unexpected error when getting the last term (%s)",
                  err.String().c_str());
    }
    return t;
  }

  std::pair<std::vector<raftpb::Entry>, Error> entries(uint64_t i,
                                                       uint64_t maxSize) {
    if (i > lastIndex()) {
      return {{}, {}};
    }
  ...
  bool maybeCommit(uint64_t maxIndex, uint64_t te) {
    ...
    // least at term 1. But if it is 0 for some reason, we don't want to
    // consider this a term match in case zeroTermOnOutOfBounds returns 0.
    if (maxIndex > committed_ && te != 0 &&
        zeroTermOnOutOfBounds(term(maxIndex)) == te) {
      commitTo(maxIndex);
      return true;
    }
    return false;
  ...
  void restore(raftpb::Snapshot s) {
    ERAFT_INFO("log [] starts to restore snapshot [index: %d, term: %d]",
...
// message size.
inline std::shared_ptr<RaftLog> newLogWithSize(
    const std::shared_ptr<Storage>& storage, uint64_t maxApplyingEntsSize) {
  if (storage == nullptr) {
...
// latest snapshot.
inline std::shared_ptr<RaftLog> newLog(
