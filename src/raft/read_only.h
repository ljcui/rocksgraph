...
struct ReadState {
  ...
  ReadState(uint64_t i, std::string ctx)
      : index_(i), requestCtx_(std::move(ctx)) {}
...
struct ReadOnly {
  ...
  std::unordered_map<std::string, std::shared_ptr<detail::readIndexStatus>>
      pendingReadIndex_;
  ...
  std::vector<std::shared_ptr<detail::readIndexStatus>> advance(
      const raftpb::Message& m) {
    int64_t i = 0;
    bool found = false;
    ...
    if (found) {
      readIndexQueue_.erase(readIndexQueue_.begin(),
                            readIndexQueue_.begin() + i);
      for (auto& rs : rss) {
