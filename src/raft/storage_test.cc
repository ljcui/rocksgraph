...
TEST(storage, TestStorageTerm) {
  std::vector<raftpb::Entry> ents = {EntryHelper{.index = 3, .term = 3}.Done(),
  ...
  struct Test {
    ...
    Test(uint64_t arg_i, Error arg_werr, uint64_t arg_wterm, bool arg_wpanic)
        : i(arg_i),
          werr(std::move(arg_werr)),
          wterm(arg_wterm),
          wpanic(arg_wpanic) {}
  };

  std::vector<Test> tests = {
      {2, ErrCompacted, 0, false},   {3, Error(nullptr), 3, false},
      {4, Error(nullptr), 4, false}, {5, Error(nullptr), 5, false},
...
TEST(storage, TestStorageEntries) {
  std::vector<raftpb::Entry> ents = {EntryHelper{.index = 3, .term = 3}.Done(),
                                     EntryHelper{.index = 4, .term = 4}.Done(),
                                     EntryHelper{.index = 5, .term = 5}.Done(),
                                     EntryHelper{.index = 6, .term = 6}.Done()};
  struct Test {
    uint64_t lo;
    uint64_t hi;
    uint64_t maxsize;
    ...
    Test(uint64_t arg_lo, uint64_t arg_hi, uint64_t arg_maxsize, Error err,
         std::vector<raftpb::Entry> entries)
        : lo(arg_lo),
          hi(arg_hi),
          maxsize(arg_maxsize),
          werr(std::move(err)),
          wentries(std::move(entries)) {}
  };
...
TEST(storage, TestStorageLastIndex) {
  ...
  s.Append(
      std::vector<raftpb::Entry>{EntryHelper{.index = 6, .term = 5}.Done()});
  std::tie(last, err) = s.LastIndex();
  EXPECT_EQ(err, nullptr);
  EXPECT_EQ(last, 6);
}
...
TEST(storage, TestStorageCompact) {
  ...
  std::vector<Test> tests = {{2, ErrCompacted, 3, 3, 3},
...
TEST(storage, TestStorageCreateSnapshot) {
  ...
  *cs.mutable_voters() = {voters.begin(), voters.end()};
  ...
  std::vector<Test> tests = {
      {4, Error(nullptr),
       SnapshotHelper{.data = data, .index = 4, .term = 4, .cs = cs}.Done()},
      {5, Error(nullptr),
       SnapshotHelper{.data = data, .index = 5, .term = 5, .cs = cs}.Done()},
  };
  ...
  for (auto& tt : tests) {
    ...
    EXPECT_EQ(err, tt.werr);
    EXPECT_TRUE(
        google::protobuf::util::MessageDifferencer::Equals(snap, tt.wsnap));
  }
}
...
TEST(storage, TestStorageAppend) {
  ...
  struct Test {
    ...
    Error werr;
    std::vector<raftpb::Entry> wentries;
    Test(std::vector<raftpb::Entry> _entries, Error _werr,
         std::vector<raftpb::Entry> _wentries)
        : entries(std::move(_entries)),
          werr(std::move(_werr)),
          wentries(std::move(_wentries)) {}
  };
  std::vector<Test> tests = {
      ...
      {{EntryHelper{.index = 3, .term = 3}.Done(),
       ...
       {EntryHelper{.index = 3, .term = 3}.Done(),
        ...
        EntryHelper{.index = 5, .term = 5}.Done()}},
      {{EntryHelper{.index = 3, .term = 3}.Done(),
        EntryHelper{.index = 4, .term = 6}.Done(),
        EntryHelper{.index = 5, .term = 6}.Done()},
       Error(nullptr),
       {EntryHelper{.index = 3, .term = 3}.Done(),
        EntryHelper{.index = 4, .term = 6}.Done(),
        EntryHelper{.index = 5, .term = 6}.Done()}},
