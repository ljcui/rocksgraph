...
enum SnapshotStatus { SnapshotFinish = 1, SnapshotFailure = 2 };

// MustSync returns true if the hard state and count of Raft entries indicate
// that a synchronous write to persistent storage is required.
inline bool MustSync(const raftpb::HardState& st,
                     const raftpb::HardState& prevst, size_t entsnum) {
...
// The methods of this struct correspond to the methods of Node and are
...
struct RawNode {
  ...
  // invoke Tick or TickQuiesced depending on whether the group is "active" or
  // "quiesced".
  //
  ...
  void TickQuiesced() { raft_->electionElapsed_++; }
  ...
  // call this when it applies a configuration change, except when it decides to
  // reject the configuration change, in which case no call must take place.
  std::shared_ptr<raftpb::ConfState> ApplyConfChange(
      const raftpb::ConfChangeI& cc) {
    return std::make_shared<raftpb::ConfState>(
        raft_->applyConfChange(cc.AsV2()));
  }
  ...
  Error Step(raftpb::Message m) {
    ...
    if (IsResponseMsg(m.type()) && !IsLocalMsgTarget(m.from()) &&
        raft_->trk_.progress_.data().count(m.from()) == 0) {
  ...
  // stable storage.
  bool applyUnstableEntries() const { return !asyncStorageWrites_; }

  bool needStorageAppendMsg(const std::shared_ptr<detail::raft>& raft,
                            const Ready& rd) {
    ...
    return !rd.entries_.empty() || !IsEmptyHardState(rd.hardState_) ||
           !IsEmptySnap(rd.snapshot_) || !raft->msgsAfterAppend_.empty();
  }

  bool needStorageAppendRespMsg(const std::shared_ptr<detail::raft>& raft,
                                const Ready& rd) {
  ...
  // after the unstable log entries, hard state, and snapshot in the current
  // Ready (along with those in all prior Ready structs) have been saved to
  // stable storage.
  raftpb::Message newStorageAppendRespMsg(
      const std::shared_ptr<detail::raft>& raft, const Ready& rd) {
    raftpb::Message m;
    ...
    if (raft->raftLog_->hasNextOrInProgressUnstableEnts()) {
      // If the raft log has unstable entries, attach the last index and term of
      // the append to the response message. This (index, term) tuple will be
      // handed back and consulted when the stability of those log entries is
      // signaled to the unstable. If the (index, term) match the unstable log
      // by the time the response is received (unstable.stableTo), the unstable
      // log can be truncated.
      //
      // However, with just this logic, there would be an ABA problem[^1] that
      // could lead to the unstable log and the stable log getting out of sync
      // temporarily and leading to an inconsistent view. Consider the following
      ...
      //  5. C proposes some log entries and B receives these entries,
      //  overwriting the
      //     previous unstable log entries that are in the process of being
      //     appended. The entries have a larger term than the previous entries
      //     but the same indexes. It begins appending these new entries
      //     asynchronously.
      //  6. C crashes and A restarts and becomes leader again after getting the
      //  vote
      //     from D and E.
      //  7. B receives the entries from A which are the same as the ones from
      //  step 2,
      //     overwriting the previous unstable log entries that are in the
      //     process of being appended from step 5. The entries have the
      //     original terms and indexes from step 2. Recall that log entries
      //     retain their original term numbers when a leader replicates entries
      //     from previous terms. It begins appending these new entries
      //     asynchronously.
      //  8. The asynchronous log appends from the first Ready complete and
      //  stableTo
      //     is called.
      //  9. However, the log entries from the second Ready are still in the
      //     asynchronous append pipeline and will overwrite (in stable storage)
      //     the entries from the first Ready at some future point. We can't
      //     truncate the unstable log yet or a future read from Storage might
      //     see the entries from step 5 before they have been replaced by the
      //     entries from step 7. Instead, we must wait until we are sure that
      //     the entries are stable and that no in-progress appends might
      //     overwrite them before removing entries from the unstable log.
      //
      // To prevent these kinds of problems, we also attach the current term to
      // the MsgStorageAppendResp (above). If the term has changed by the time
      // the MsgStorageAppendResp if returned, the response is ignored and the
      // unstable log is not truncated. The unstable log is only truncated when
      // the term has remained unchanged from the time that the MsgStorageAppend
      // was sent to the time that the MsgStorageAppendResp is received,
      // indicating that no-one else is in the process of truncating the stable
      // log.
      //
      // However, this replaces a correctness problem with a liveness problem.
      // If we only attempted to truncate the unstable log when appending new
      // entries but also occasionally dropped these responses, then quiescence
      // of new log entries could lead to the unstable log never being
      // truncated.
      //
      // To combat this, we attempt to truncate the log on all
      // MsgStorageAppendResp messages where the unstable log is not empty, not
      // just those associated with entry appends. This includes
      // MsgStorageAppendResp messages associated with an updated HardState,
      // which occur after a term change.
      //
      ...
      //  if len(rd.Entries) > 0 { ... }
      //
      // To do so, we attach r.raftLog.lastIndex() and r.raftLog.lastTerm(), not
      // the (index, term) of the last entry in rd.Entries. If rd.Entries is not
      // empty, these will be the same. However, if rd.Entries is empty, we
      // still want to attest that this (index, term) is correct at the current
      // term, in case the MsgStorageAppend that contained the last entry in the
      // unstable slice carried an earlier term and was dropped.
      //
      // A MsgStorageAppend with a new term is emitted on each term change. This
      // is the same condition that causes MsgStorageAppendResp messages with
      // earlier terms to be ignored. As a result, we are guaranteed that,
      // assuming a bounded number of term changes, there will eventually be a
      // MsgStorageAppendResp message that is not ignored. This means that
      // entries in the unstable log which have been appended to stable storage
      // will eventually be truncated and dropped from memory.
      //
  ...
  raftpb::Message newStorageAppendMsg(std::shared_ptr<detail::raft> r,
                                      const Ready& rd) {
    raftpb::Message m;
    ...
    *m.mutable_responses() = {raft_->msgsAfterAppend_.begin(),
                              raft_->msgsAfterAppend_.end()};
  ...
  raftpb::Message newStorageApplyRespMsg(
      const std::shared_ptr<detail::raft>& raft,
    ...
    *m.mutable_entries() = {std::make_move_iterator(ents.begin()),
  ...
  raftpb::Message newStorageApplyMsg(const std::shared_ptr<detail::raft>& raft,
                                     const Ready& rd) {
  ...
  Ready readyWithoutAccept() {
    ...
    rd.committedEntries_ =
        raft_->raftLog_->nextCommittedEnts(applyUnstableEntries());
    ...
    rd.mustSync_ =
        MustSync(raft_->hardState(), prevHardSt_, rd.entries_.size());
  ...
  void acceptReady(const Ready& rd) {
    ...
    if (!rd.committedEntries_.empty()) {
      ...
      raft_->raftLog_->acceptApplying(index, entsSize(ents),
                                      applyUnstableEntries());
    }
  }
  ...
  // HardState, and sending messages. The returned Ready() *must* be handled and
  // subsequently passed back via Advance().
  Ready GetReady() {
  ...
  bool HasReady() {
    ...
    if (raft_->raftLog_->hasNextUnstableEnts() ||
        raft_->raftLog_->hasNextCommittedEnts(applyUnstableEntries())) {
      return true;
    }
    if (!raft_->readStates_.empty()) {
      return true;
  ...
  //
  // NOTE: Advance must not be called when using AsyncStorageWrites. Response
  // messages from the local append and apply threads take its place.
  ...
  // GetStatus returns the current status of the given group. This allocates,
  // see BasicStatus and WithProgress for allocation-friendlier choices.
  Status GetStatus() {
  ...
  BasicStatus GetBasicStatus() { return detail::getBasicStatus(*raft_); }

  // WithProgress is a helper to introspect the Progress for this node and its
  // peers.
  void WithProgress(
      std::function<void(uint64_t, ProgressType, tracker::Progress)>& visitor) {
    raft_->trk_.Visit(
        [visitor](uint64_t id, std::shared_ptr<tracker::Progress>& pr) {
  ...
  // read index, any linearizable read requests issued before the read request
  ...
  // is nonempty.
  //
  // It is recommended that instead of calling this method, applications
  // bootstrap their state manually by setting up a Storage that has a first
  // index > 1 and which stores the desired ConfState as its InitialState.
