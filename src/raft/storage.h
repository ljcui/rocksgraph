...
const Error ErrUnavailable(Error::Unavailable,
...
const Error ErrSnapshotTemporarilyUnavailable(
...
struct Storage {
  ...
  virtual std::tuple<raftpb::HardState, raftpb::ConfState, Error>
  InitialState() = 0;
  ...
  virtual std::pair<std::vector<raftpb::Entry>, Error> Entries(
      uint64_t lo, uint64_t hi, uint64_t maxSize) = 0;
...
struct MemoryStorage : public Storage {
 ...
 public:
  ...
  uint64_t lastIndex() { return ents_[0].index() + ents_.size() - 1; }

 public:
  MemoryStorage() = default;
  explicit MemoryStorage(std::vector<raftpb::Entry> ents)
      : ents_(std::move(ents)) {}

  // InitialState implements the Storage interface.
  std::tuple<raftpb::HardState, raftpb::ConfState, Error> InitialState()
      override {
    callStats_.initialState++;
    return std::make_tuple(hardState_, snapshot_.metadata().conf_state(),
                           Error());
  }
  ...
  std::pair<std::vector<raftpb::Entry>, Error> Entries(
      uint64_t lo, uint64_t hi, uint64_t maxSize) override {
    std::lock_guard<std::mutex> lockGuard(mutex_);
    callStats_.entries++;
    ...
    if (hi > lastIndex() + 1) {
      ERAFT_FATAL("entries' hi(%d) is out of bound lastindex(%d)", hi,
                  lastIndex());
    }
    ...
    std::vector<raftpb::Entry> ret(ents_.begin() + (int64_t)(lo - offset),
                                   ents_.begin() + (int64_t)(hi - offset));
  ...
  uint64_t firstIndex() { return ents_[0].index() + 1; }
  ...
  std::pair<raftpb::Snapshot, Error> CreateSnapshot(uint64_t i,
                                                    raftpb::ConfState* cs,
                                                    std::string data) {
    std::lock_guard<std::mutex> lockGuard(mutex_);
  ...
  Error Compact(uint64_t compactIndex) {
    ...
    if (compactIndex > lastIndex()) {
      ERAFT_FATAL("compact %d is out of bound lastindex(%d)", compactIndex,
                  lastIndex());
    }
  ...
  Error Append(std::vector<raftpb::Entry> entries) {
    ...
    if (ents_.size() > offset) {
      ...
      ERAFT_FATAL("missing log entry [last: %d, append at: %d]", lastIndex(),
                  entries[0].index());
    }
    return nullptr;
  }
