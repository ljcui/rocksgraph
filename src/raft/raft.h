...
struct SoftState {
  uint64_t lead_ =
      0;  // must use atomic operations to access; keep 64-bit aligned.
  StateType raftState_ = StateFollower;
  SoftState() = default;
  SoftState(uint64_t lead, StateType raftState)
      : lead_(lead), raftState_(raftState) {}
...
struct Ready {
  ...
  bool operator==(const Ready& r) const {
    ...
    if (!google::protobuf::util::MessageDifferencer::Equals(hardState_,
                                                            r.hardState_)) {
      return false;
    }
    ...
    if (!google::protobuf::util::MessageDifferencer::Equals(snapshot_,
                                                            r.snapshot_)) {
      return false;
    }
  ...
  bool containsUpdates() const {
    return softState_ != nullptr || !IsEmptyHardState(hardState_) ||
           ...
           !committedEntries_.empty() || !messages_.empty() ||
           !readStates_.empty();
  }
...
struct raft {
  ...
  std::tuple<size_t, size_t, quorum::VoteResult> poll(uint64_t id,
  ...
  raftpb::ConfState switchToConfig(tracker::Config cfg,
  ...
  raftpb::Message responseToReadIndexReq(raftpb::Message req,
                                         uint64_t readIndex);
...
inline std::shared_ptr<raft> newRaft(Config& c) {
  ...
  auto raftlog =
      detail::newLogWithSize(c.storage_, c.maxCommittedSizePerReady_);
  raftpb::HardState hs;
  raftpb::ConfState cs;
  ...
  r->trk_ =
      tracker::MakeProgressTracker(c.maxInflightMsgs_, c.maxInflightBytes_);
  ...
  tracker::ProgressMap prs;
  std::tie(cfg, prs, err) =
      confchange::Restore({r->trk_, raftlog->lastIndex()}, cs);
  if (err != nullptr) {
    ERAFT_FATAL(err.String().c_str());
  }
  ...
  ERAFT_INFO(
      "newRaft %x [peers: [%s], term: %llu, commit: %llu, applied: %llu, "
      "lastindex: %llu, lastterm: %llu]",
      r->id_, Join(nodesStrs, ",").c_str(), r->term_, r->raftLog_->committed_,
      r->raftLog_->applied_, r->raftLog_->lastIndex(), r->raftLog_->lastTerm());
  return r;
}

inline bool raft::hasLeader() { return lead_ != None; }

inline SoftState raft::softState() { return {lead_, state_}; }
...
inline void raft::send(raftpb::Message m) {
  ...
  if (m.type() == raftpb::MsgVote || m.type() == raftpb::MsgVoteResp ||
      m.type() == raftpb::MsgPreVote || m.type() == raftpb::MsgPreVoteResp) {
    if (m.term() == 0) {
      ...
      ERAFT_FATAL("term should be set when sending %s",
                  raftpb::MessageType_Name(m.type()).c_str());
    }
  } else {
    if (m.term() != 0) {
      ERAFT_FATAL("term should not be set when sending %s (was %d)",
                  raftpb::MessageType_Name(m.type()).c_str(), m.term());
    }
  ...
  if (m.type() == raftpb::MsgAppResp || m.type() == raftpb::MsgVoteResp ||
...
inline void raft::sendAppend(uint64_t to) { maybeSendAppend(to, true); }
...
inline bool raft::maybeSendAppend(uint64_t to, bool sendIfEmpty) {
  ...
  if (errt != nullptr ||
      erre != nullptr) {  // send snapshot if we failed to get term or entries
    if (!pr->recentActive_) {
      ERAFT_DEBUG(
    ...
    if (err != nullptr) {
      if (err == ErrSnapshotTemporarilyUnavailable) {
        ERAFT_DEBUG(
            "%x failed to send snapshot to %x because snapshot is temporarily "
            "unavailable",
            id_, to);
        return false;
      }
    ...
    ERAFT_DEBUG(
        "%x [firstindex: %d, commit: %d] sent snapshot[index: %d, term: %d] to "
        "%x [%s]",
        id_, raftLog_->firstIndex(), raftLog_->committed_, sindex, sterm, to,
        pr->String().c_str());
    pr->BecomeSnapshot(sindex);
    ERAFT_DEBUG("%x paused sending replication messages to %x [%s]", id_, to,
                pr->String().c_str());
    raftpb::Message msg;
    msg.set_to(to);
  ...
  *msg.mutable_entries() = {std::make_move_iterator(ents.begin()),
...
inline void raft::appliedTo(uint64_t index, uint64_t size) {
  ...
  if (trk_.config_.autoLeave_ && newApplied >= pendingConfIndex_ &&
      state_ == StateLeader) {
...
inline void raft::reset(uint64_t term) {
  ...
  trk_.Visit([this](uint64_t id, std::shared_ptr<tracker::Progress>& pr) {
    ...
    new_pr->inflights_ =
        tracker::NewInflights(trk_.maxInflight_, trk_.maxInflightBytes_);
...
inline bool raft::appendEntry(std::vector<raftpb::Entry> es) {
  ...
  if (!increaseUncommittedSize(es)) {
    ERAFT_WARN(
        "%x appending new entries to log would exceed uncommitted entry size "
...
inline void raft::tickHeartbeat() {
  ...
  if (electionElapsed_ >= electionTimeout_) {
    ...
    if (checkQuorum_) {
      ...
      auto err = Step(std::move(m));
      if (err != nullptr) {
        ERAFT_DEBUG("error occurred during checking sending heartbeat: %s",
                    err.String().c_str());
      }
    }
...
inline bool raft::hasUnappliedConfChanges() {
  if (raftLog_->applied_ >=
      raftLog_->committed_) {  // in fact applied == committed
    return false;
  }
  auto found = false;
  ...
  auto err = raftLog_->scan(
      lo, hi, pageSize, [&](const std::vector<raftpb::Entry>& ents) -> Error {
        for (size_t i = 0; i < ents.size(); i++) {
          if (ents[i].type() == raftpb::EntryConfChange ||
              ents[i].type() == raftpb::EntryConfChangeV2) {
            found = true;
            return errBreak;
          };
        }
  ...
  if (err != nullptr && err != errBreak) {
    ERAFT_FATAL("error scanning unapplied entries [%d, %d): %s", lo, hi,
                err.String().c_str());
  }
  return found;
}
...
inline void raft::campaign(const CampaignType& t) {
  ...
  for (auto id : ids) {
    ...
    ERAFT_INFO("%x [logterm: %d, index: %d] sent %s request to %x at term %d",
               ...
               MessageType_Name(voteMsg).c_str(), id, term_);
...
inline std::tuple<size_t, size_t, quorum::VoteResult> raft::poll(
    uint64_t id, raftpb::MessageType t, bool v) {
  if (v) {
    ERAFT_INFO("%x received %s from %x at term %d", id_,
               MessageType_Name(t).c_str(), id, term_);
  } else {
    ERAFT_INFO("%x received %s rejection from %x at term %d", id_,
               MessageType_Name(t).c_str(), id, term_)
  }
...
inline Error raft::Step(raftpb::Message m) {
  ...
  if (m.term() == 0) {
    ...
    if (m.type() == raftpb::MsgVote || m.type() == raftpb::MsgPreVote) {
      ...
      auto inLease =
          checkQuorum_ && lead_ != None && electionElapsed_ < electionTimeout_;
      if (!force && inLease) {
        ...
        ERAFT_INFO(
            ...
            MessageType_Name(m.type()).c_str(), m.from(), m.logterm(),
    ...
    if (m.type() == raftpb::MsgPreVote) {
      ...
      ERAFT_INFO(
          "%x [term: %d] received a %s message with higher term from %x [term: "
          "%d]",
    ...
    if ((checkQuorum_ || preVote_) &&
        (m.type() == raftpb::MsgHeartbeat || m.type() == raftpb::MsgApp)) {
      ...
      ERAFT_INFO(
          "%x [logterm: %d, index: %d, vote: %x] rejected %s from %x [logterm: "
          "%d, index: %d] at term %d",
          id_, raftLog_->lastTerm(), raftLog_->lastIndex(), vote_,
          MessageType_Name(m.type()).c_str(), m.from(), m.logterm(), m.index(),
          term_);
      raftpb::Message msg;
      msg.set_to(m.from());
      msg.set_term(term_);
      ...
      if (m.index() != 0) {
        ...
        // about this race.
        ERAFT_INFO(
            "%x [term: %d] ignored entry appends from a %s message with lower "
            "term [term: %d]",
      ...
      // ignore other cases
      ERAFT_INFO(
          "%x [term: %d] ignored a %s message with lower term from %x [term: "
          "%d]",
          id_, term_, MessageType_Name(m.type()).c_str(), m.from(), m.term());
    }
    return {};
  ...
  switch (m.type()) {
    ...
    case raftpb::MsgPreVote: {
      ...
      if (canVote && raftLog_->isUpToDate(m.index(), m.logterm())) {
        ...
        // learned about this yet. For example, consider a group in which id=1
        // is a learner and id=2 and id=3 are voters. A configuration change
        ...
        // continue to do so until 3 has stepped up as leader, replicates the
        // conf change to 1, and 1 applies it. Ultimately, by receiving a
        // request to vote, the learner realizes that the candidate believes it
        // to be a voter, and that it should act accordingly. The candidate's
        // config may be stale, too; but in that case it won't win the election,
        // at least in the absence of the bug discussed in:
        // https://github.com/etcd-io/etcd/issues/7625#issuecomment-488798263.
        ERAFT_INFO(
            "%x [logterm: %d, index: %d, vote: %x] cast %s for %x [logterm: "
            "%d, index: %d] at term %d",
            id_, raftLog_->lastTerm(), raftLog_->lastIndex(), vote_,
            MessageType_Name(m.type()).c_str(), m.from(), m.logterm(),
            m.index(), term_);
...
inline Error stepLeader(raft& r, raftpb::Message m) {
  ...
  switch (m.type()) {
    ...
    case raftpb::MsgCheckQuorum: {
      if (!r.trk_.QuorumActive()) {
        ERAFT_WARN("%x stepped down to follower since quorum is not active",
                   r.id_);
        r.becomeFollower(r.term_, None);
      }
    ...
    case raftpb::MsgProp: {
      ...
      if (r.leadTransferee_ != None) {
        ERAFT_DEBUG(
            "%x [term %d] transfer leadership to %x is in progress; dropping "
            "proposal",
      ...
      for (int i = 0; i < m.entries().size(); i++) {
        ...
        if (cc != nullptr) {
          ...
          if (!failedCheck.empty() && !r.disableConfChangeValidation_) {
            ERAFT_INFO("%x ignoring conf change [%s] at config %s: %s", r.id_,
                       cc->String().c_str(), r.trk_.config_.String().c_str(),
                       failedCheck.c_str());
            raftpb::Entry ent;
  ...
  switch (m.type()) {
    case raftpb::MsgAppResp: {
      // NB: this code path is also hit from (*raft).advance, where the leader
      // steps an MsgAppResp to acknowledge the appended entries in the last
      // Ready.
      pr->recentActive_ = true;
      ...
      if (m.reject()) {
        ...
        // described below.
        ERAFT_DEBUG(
            "%x received MsgAppResp(rejected, hint: (index %d, term %d)) from "
            "%x for index %d",
        ...
        if (m.logterm() > 0) {
          ...
          //    log.
          std::tie(nextProbeIdx, std::ignore) =
              r.raftLog_->findConflictByTerm(m.rejecthint(), m.logterm());
        }
        if (pr->MaybeDecrTo(m.index(), nextProbeIdx)) {
          ERAFT_DEBUG("%x decreased progress of %x to [%s]", r.id_, m.from(),
                      pr->String().c_str());
          if (pr->state_ == tracker::StateType::StateReplicate) {
            pr->BecomeProbe();
          }
        ...
        if (pr->MaybeUpdate(m.index()) ||
            (pr->match_ == m.index() &&
             pr->state_ == tracker::StateType::StateProbe)) {
          if (pr->state_ == tracker::StateType::StateProbe) {
            pr->BecomeReplicate();
          } else if (pr->state_ == tracker::StateType::StateSnapshot &&
                     pr->match_ + 1 >= r.raftLog_->firstIndex()) {
          ...
          if (m.from() == r.leadTransferee_ &&
              pr->match_ == r.raftLog_->lastIndex()) {
            ERAFT_INFO("%x sent MsgTimeoutNow to %x after received MsgAppResp",
                       r.id_, m.from());
    ...
    case raftpb::MsgHeartbeatResp: {
      ...
      //
      // If the follower is fully caught up but also in StateProbe (as can
      // happen if ReportUnreachable was called), we also want to send an append
      // (it will be empty) to allow the follower to transition back to
      // StateReplicate once it responds.
      //
      ...
      if (r.trk_.config_.voters_.GetVoteResult(r.readOnly_->recvAck(
              m.from(), m.context())) != quorum::VoteResult::VoteWon) {
        return {};
      }
    ...
    case raftpb::MsgSnapStatus: {
      ...
      if (!m.reject()) {
        ...
        pr->BecomeProbe();
        ERAFT_DEBUG(
            "%x snapshot failed, resumed sending replication messages to %x "
            "[%s]",
            r.id_, m.from(), pr->String().c_str());
      }
    ...
    case raftpb::MsgTransferLeader: {
      ...
      if (lastLeadTransferee != None) {
        if (lastLeadTransferee == leadTransferee) {
          ERAFT_INFO(
              "%x [term %d] transfer leadership to %x is in progress, ignores "
              "request to same node %x",
              r.id_, r.term_, leadTransferee, leadTransferee);
          return {};
        }
        r.abortLeaderTransfer();
        ERAFT_INFO("%x [term %d] abort previous transferring leadership to %x",
                   r.id_, r.term_, lastLeadTransferee);
      }
      ...
      ERAFT_INFO("%x [term %d] starts to transfer leadership to %x", r.id_,
                 r.term_, leadTransferee);
...
inline Error stepCandidate(raft& r, raftpb::Message m) {
  switch (m.type()) {
    ...
    case raftpb::MsgVoteResp: {
      ...
      ERAFT_INFO("%x has received %d %s votes and %d vote rejections", r.id_,
                 gr, MessageType_Name(m.type()).c_str(), rj);
      switch (res) {
    ...
    case raftpb::MsgTimeoutNow: {
      ERAFT_DEBUG("%x [term %d state %v] ignored MsgTimeoutNow from %x", r.id_,
                  r.term_, r.state_, m.from());
      break;
    }
    default: {
      break;
...
inline Error stepFollower(raft& r, raftpb::Message m) {
  switch (m.type()) {
    case raftpb::MsgProp: {
      if (r.lead_ == None) {
        ERAFT_INFO("%x no leader at term %d; dropping proposal", r.id_,
                   r.term_);
        return ErrProposalDropped;
    ...
    case raftpb::MsgTransferLeader: {
      if (r.lead_ == None) {
        ERAFT_INFO("%x no leader at term %d; dropping leader transfer msg",
                   r.id_, r.term_);
        return {};
      }
      m.set_to(r.lead_);
    ...
    case raftpb::MsgForgetLeader: {
      ...
      if (r.lead_ != None) {
        ERAFT_INFO("%x forgetting leader %x at term %d", r.id_, r.lead_,
                   r.term_);
        r.lead_ = None;
      }
      break;
    ...
    case raftpb::MsgReadIndex: {
      if (r.lead_ == None) {
        ERAFT_INFO("%x no leader at term %d; dropping index reading msg", r.id_,
                   r.term_);
        return {};
      }
      m.set_to(r.lead_);
...
inline void raft::handleAppendEntries(raftpb::Message m) {
  ...
  bool ok = false;
  std::tie(mlastIndex, ok) =
      raftLog_->maybeAppend(m.index(), m.logterm(), m.commit(),
  ...
  ERAFT_DEBUG(
      "%x [logterm: %d, index: %d] rejected MsgApp [logterm: %d, index: %d] "
      "from %x",
      id_, raftLog_->zeroTermOnOutOfBounds(raftLog_->term(m.index())),
  ...
  // a non-zero term (unless the log is empty). However, it is safe to send a
  // zero LogTerm in this response in any case, so we don't verify it here.
  auto hintIndex = std::min(m.index(), raftLog_->lastIndex());
  uint64_t hintTerm = 0;
  std::tie(hintIndex, hintTerm) =
      raftLog_->findConflictByTerm(hintIndex, m.logterm());
  raftpb::Message msg;
  msg.set_to(m.from());
  msg.set_type(raftpb::MsgAppResp);
...
inline void raft::handleSnapshot(raftpb::Message m) {
  ...
  if (restore(std::move(s))) {
    ERAFT_INFO("%x [commit: %d] restored snapshot [index: %d, term: %d]", id_,
               raftLog_->committed_, sindex, sterm);
    raftpb::Message msg;
    msg.set_to(m.from());
    msg.set_type(raftpb::MsgAppResp);
    ...
    ERAFT_INFO("%x [commit: %d] ignored snapshot [index: %d, term: %d]", id_,
               raftLog_->committed_, sindex, sterm);
    raftpb::Message msg;
    msg.set_to(m.from());
    msg.set_type(raftpb::MsgAppResp);
...
// snapshot was ignored, either because it was obsolete or because of an error.
inline bool raft::restore(raftpb::Snapshot s) {
  ...
  if (state_ != StateFollower) {
    ...
    ERAFT_WARN(
        "%x attempted to restore snapshot as leader; should never happen", id_);
  ...
  for (auto& set : std::vector<std::vector<uint64_t>>{
           {cs.voters().begin(), cs.voters().end()},
  ...
  if (raftLog_->matchTerm(snapshot_index, snapshot_term)) {
    ERAFT_INFO(
        "%x [commit: %d, lastindex: %d, lastterm: %d] fast-forwarded commit to "
  ...
  trk_ =
      tracker::MakeProgressTracker(trk_.maxInflight_, trk_.maxInflightBytes_);
  tracker::Config cfg;
  tracker::ProgressMap prs;
  Error err;
  std::tie(cfg, prs, err) =
      confchange::Restore({trk_, raftLog_->lastIndex()}, cs);
  if (err != nullptr) {
    // This should never happen. Either there's a bug in our config change
    // handling or the client corrupted the conf change.
    ERAFT_FATAL("unable to restore config [%s]: %s",
                cs.SerializeAsString().c_str(), err.String().c_str());
  }
  ...
  auto& pr = trk_.progress_[id_];
  pr->MaybeUpdate(pr->next_ -
                  1);  // TODO(tbg): this is untested and likely unneeded

  ERAFT_INFO(
      "%x [commit: %d, lastindex: %d, lastterm: %d] restored snapshot [index: "
      "%d, term: %d]",
      id_, raftLog_->committed_, raftLog_->lastIndex(), raftLog_->lastTerm(),
...
inline raftpb::ConfState raft::applyConfChange(raftpb::ConfChangeV2 cc) {
  ...
  std::tie(cfg, prs, err) = [this, &ccv2_wrap]() {
    ...
    if (ok) {
      return changer.EnterJoint(autoLeave, {ccv2_wrap.data().changes().begin(),
                                            ccv2_wrap.data().changes().end()});
    }
    return changer.Simple(
        {ccv2_wrap.data().changes().begin(), ccv2_wrap.data().changes().end()});
  }();
...
// ConfChange.
inline raftpb::ConfState raft::switchToConfig(tracker::Config cfg,
                                              tracker::ProgressMap prs) {
  trk_.config_ = std::move(cfg);
  trk_.progress_ = std::move(prs);

  ERAFT_INFO("%x switched to configuration %s", id_,
             trk_.config_.String().c_str())
  ...
  // transfer.
  if (trk_.config_.voters_.IDs().find(leadTransferee_) ==
          trk_.config_.voters_.IDs().end() &&
      leadTransferee_ != 0) {
    abortLeaderTransfer();
  }
...
inline void raft::loadState(const raftpb::HardState& state) {
  if (state.commit() < raftLog_->committed_ ||
      state.commit() > raftLog_->lastIndex()) {
    ERAFT_FATAL("%x state.commit %d is out of range [%d, %d]", id_,
                state.commit(), raftLog_->committed_, raftLog_->lastIndex());
  }
...
inline void raft::abortLeaderTransfer() { leadTransferee_ = None; }
...
inline bool raft::committedEntryInCurrentTerm() {
  ...
  return raftLog_->zeroTermOnOutOfBounds(
             raftLog_->term(raftLog_->committed_)) == term_;
}

// responseToReadIndexReq constructs a response for `req`. If `req` comes from
// the peer itself, a blank value will be returned.
inline raftpb::Message raft::responseToReadIndexReq(raftpb::Message req,
                                                    uint64_t readIndex) {
...
inline bool raft::increaseUncommittedSize(
    const std::vector<raftpb::Entry>& ents) {
  auto s = payloadsSize(ents);
  if (uncommittedSize_ > 0 && s > 0 &&
      uncommittedSize_ + s > maxUncommittedSize_) {
...
inline void sendMsgReadIndexResponse(raft& r, raftpb::Message m) {
  ...
  switch (r.readOnly_->option_) {
    ...
    case ReadOnlyLeaseBased: {
      auto resp =
          r.responseToReadIndexReq(std::move(m), r.raftLog_->committed_);
      if (resp.to() != None) {
        r.send(std::move(resp));
      }
      break;
