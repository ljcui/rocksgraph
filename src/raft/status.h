...
struct Status {
  ...
  std::pair<std::string, Error> MarshalJSON() {
    auto j = format(
        R"({"id":"%x","term":%d,"vote":"%x","commit":%d,"lead":"%x","raftState":"%s","applied":%d,"progress":{)",
        basicStatus_.id_, basicStatus_.hardState_.term(),
        basicStatus_.hardState_.vote(), basicStatus_.hardState_.commit(),
    ...
    if (progress_.empty()) {
      ...
      for (auto& pair : progress_) {
        auto subj = format(R"("%x":{"match":%d,"next":%d,"state":"%s"},)",
                           pair.first, pair.second.match_, pair.second.next_,
...
namespace detail {
inline std::unordered_map<uint64_t, tracker::Progress> getProgressCopy(
    raft& r) {
