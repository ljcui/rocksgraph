...
struct Unstable {
  ...
  // nextSnapshot returns the unstable snapshot, if one exists that is not
  // already in the process of being written to storage.
  ...
  // acceptInProgress marks all entries and the snapshot, if any, in the
  // unstable as having begun the process of being written to storage. The
  // entries/snapshot will no longer be returned from nextEntries/nextSnapshot.
  // However, new entries/snapshots added after a call to acceptInProgress will
  // be returned from those methods, until the next call to acceptInProgress.
  ...
  void stableTo(uint64_t i, uint64_t t) {
    ...
    if (gt != t) {
      ...
      ERAFT_INFO(
          "entry at (index,term)=(%d,%d) mismatched with entry at (%d,%d) in "
          "unstable log; ignoring",
          i, t, i, gt);
      return;
    }
  ...
  void mustCheckOutOfBounds(uint64_t lo, uint64_t hi) {
    ...
    if (lo < offset_ || hi > upper) {
      ERAFT_FATAL("unstable.slice[%d,%d) out of bound [%d,%d]", lo, hi, offset_,
                  upper);
    }
  }
  ...
  // will panic. The returned slice can be appended to, but the entries in it
  // must not be changed because they are still shared with unstable.
  //
  // TODO(pavelkalinnikov): this, and similar []pb.Entry slices, may bubble up
  // all the way to the application code through Ready struct. Protect other
  // slices similarly, and document how the client can use them.
  std::vector<raftpb::Entry> slice(uint64_t lo, uint64_t hi) {
    mustCheckOutOfBounds(lo, hi);
    return {entries_.begin() + (lo - offset_),
  ...
  void truncateAndAppend(std::vector<raftpb::Entry> ents) {
    ...
    if (fromIndex == offset_ + entries_.size()) {
      ...
      entries_.insert(entries_.end(), std::make_move_iterator(ents.begin()),
