...
struct MajorityConfig {
  ...
  bool operator==(const MajorityConfig& m) const { return data_ == m.data_; }
  explicit MajorityConfig(std::unordered_set<uint64_t> d)
      : data_(std::move(d)){};
  ...
  std::string Describe(const AckedIndexer& l) {
    ...
    std::sort(info.begin(), info.end(),
              [](const tup& i, const tup& j) { return i.id < j.id; });
