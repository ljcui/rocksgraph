...
struct JointConfig {
  ...
  JointConfig() = default;
  bool operator==(const JointConfig& j) const { return data_ == j.data_; }
  // TODO(wangzhiyong): return nullptr if not exist
  MajorityConfig& operator[](size_t index) { return data_.at(index); }
  ...
  // GetVoteResult takes a mapping of voters to yes/no (true/false) votes and
