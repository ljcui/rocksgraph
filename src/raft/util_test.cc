...
TEST(util, TestLimitSize) {
  std::vector<raftpb::Entry> ents = {EntryHelper{.index = 4, .term = 4}.Done(),
  ...
  std::vector<Test> tests = {
      {std::numeric_limits<uint64_t>::max(),
       prefix(ents.size())},  // all entries are returned
      // Even if maxSize is zero, the first entry should be returned.
      {0, prefix(1)},
      // Limit to 2.
      {ents[0].ByteSizeLong() + ents[1].ByteSizeLong(), prefix(2)},
      {ents[0].ByteSizeLong() + ents[1].ByteSizeLong() +
           ents[2].ByteSizeLong() / 2,
       prefix(2)},
      {ents[0].ByteSizeLong() + ents[1].ByteSizeLong() +
           ents[2].ByteSizeLong() - 1,
       prefix(2)},
      // All.
      {ents[0].ByteSizeLong() + ents[1].ByteSizeLong() + ents[2].ByteSizeLong(),
       prefix(3)},
  };
  for (auto& tt : tests) {
    auto got = ents;
...
// TestPayloadSizeOfEmptyEntry ensures that payloadSize of empty entry is always
// zero. This property is important because new leaders append an empty entry to
// their log, and we don't want this to count towards the uncommitted log quota.
