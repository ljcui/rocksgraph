...
struct ConfChangeWrap : ConfChangeI {
  ...
  std::pair<ConfChange, bool> AsV1() const override { return {data_, true}; };
  std::string String() const override { return data_.SerializeAsString(); }
};
...
struct ConfChangeV2Wrap : ConfChangeI {
  ...
  std::pair<ConfChange, bool> AsV1() const override { return {{}, false}; };

  // EnterJoint returns two bools. The second bool is true if and only if this
  // config change will use Joint Consensus, which is the case if it contains
  // more than one change or if the use of Joint Consensus was requested
  // explicitly. The first bool can only be true if second one is, and indicates
  // whether the Joint State will be left automatically.
  std::pair<bool, bool> EnterJoint() {
    ...
    if (data_.transition() != ConfChangeTransitionAuto ||
        data_.changes().size() > 1) {
      // Use Joint Consensus.
      ...
      switch (data_.transition()) {
        ...
        case ConfChangeTransitionJointExplicit:
          break;
        default:
          ERAFT_FATAL("unknown transition: %s",
                      data_.ShortDebugString().c_str());
      }
  ...
  bool LeaveJoint() {
    // TODO(wangzhiyong)
    data_.clear_context();
    return google::protobuf::util::MessageDifferencer::Equals(data_,
                                                              ConfChangeV2());
  }
...
inline std::tuple<EntryType, std::string, Error> MarshalConfChange(
    ConfChangeI* c) {
  EntryType typ;
  std::string ccdata;
  Error err;
...
// - rn: remove n, and
// - un: update n.
inline std::pair<std::vector<ConfChangeSingle>, Error> ConfChangesFromString(
    std::string s) {
