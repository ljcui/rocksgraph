...
extern Config newTestConfig(uint64_t id, int election, int heartbeat,
...
std::shared_ptr<RawNode> newTestRawNode(uint64_t id, int election,
                                        int heartbeat,
                                        std::shared_ptr<Storage> storage) {
...
TEST(raft, TestRawNodeStep) {
  for (int i = static_cast<int>(raftpb::MessageType_MIN);
    ...
    *sp.mutable_metadata()->mutable_conf_state()->mutable_voters() = {
...
// successfully.
std::shared_ptr<raftpb::ConfChangeWrap> NewConfChange(
    raftpb::ConfChangeType type, int node_id) {
...
raftpb::ConfChangeSingle NewConfChangeSingle(raftpb::ConfChangeType type,
                                             int node_id) {
...
raftpb::ConfState NewConfState(const std::vector<uint64_t>& voters,
  ...
  *cs.mutable_voters_outgoing() = {voters_outgoing.begin(),
                                   voters_outgoing.end()};
...
std::shared_ptr<raftpb::ConfChangeV2Wrap> NewConfChangeV2(
    const std::vector<raftpb::ConfChangeSingle>& ccss) {
...
std::shared_ptr<raftpb::ConfChangeV2Wrap> NewConfChangeV2(
    const std::vector<raftpb::ConfChangeSingle>& ccss,
    raftpb::ConfChangeTransition cct) {
  raftpb::ConfChangeV2 ccv2;
  *ccv2.mutable_changes() = {ccss.begin(), ccss.end()};
...
TEST(raft, TestRawNodeProposeAndConfChange) {
  ...
  std::vector<Test> testCases = {
      ...
      {
          NewConfChangeV2(
              {NewConfChangeSingle(raftpb::ConfChangeAddLearnerNode, 2)}),
          NewConfState({1}, {2}, {}, {}, false),
          nullptr,
      },
      // We can ask explicitly for joint consensus if we want it.
      {
          ...
          std::make_shared<raftpb::ConfState>(
              NewConfState({1}, {2}, {}, {}, false)),
      },
      ...
      // which we really need joint config changes and also need LearnersNext.
      {
          ...
          std::make_shared<raftpb::ConfState>(
              NewConfState({2}, {1, 3}, {}, {}, false)),
      },
      // Ditto explicit.
      {
          NewConfChangeV2(
              {NewConfChangeSingle(raftpb::ConfChangeAddNode, 2),
               NewConfChangeSingle(raftpb::ConfChangeAddLearnerNode, 1),
               NewConfChangeSingle(raftpb::ConfChangeAddLearnerNode, 3)},
              raftpb::ConfChangeTransitionJointExplicit),
  ...
  for (auto& tc : testCases) {
    ...
    EXPECT_TRUE(
        google::protobuf::util::MessageDifferencer::Equals(tc.exp, *cs));

    uint64_t maybePlusOne = 0;
    bool autoLeave = false;
    std::tie(autoLeave, ok) =
        raftpb::ConfChangeV2Wrap(tc.cc->AsV2()).EnterJoint();
    if (ok && autoLeave) {
    ...
    EXPECT_FALSE(rd.entries_.size() != 1 ||
                 rd.entries_[0].type() != raftpb::EntryConfChangeV2);
    ...
    EXPECT_TRUE(
        google::protobuf::util::MessageDifferencer::Equals(*tc.exp2, *cs));
    rawNode->Advance(rd);
  }
}
...
TEST(raft, TestRawNodeJointAutoLeave) {
  ...
  *expCs.mutable_voters_outgoing() = {votersOutgoing.begin(),
                                      votersOutgoing.end()};
...
TEST(raft, TestRawNodeReadIndex) {
  ...
  auto equal = [](const std::vector<ReadState>& a,
                  const std::vector<ReadState>& b) {
    if (a.size() != b.size()) {
      return false;
    }
...
TEST(raft, TestRawNodeStart) {
  std::vector<raftpb::Entry> entries = {
      {EntryHelper{.index = 2, .term = 1}.Done(),
       EntryHelper{.index = 3, .term = 1, .data = "foo"}.Done()}};
  Ready want;
  ...
  auto bootstrap = [](const std::shared_ptr<MemoryStorage>& storage,
                      const raftpb::ConfState& cs) -> Error {
    if (cs.voters().empty()) {
    ...
    std::tie(std::ignore, err) =
        storage->Entries(fi, fi, std::numeric_limits<uint64_t>::max());
    if (err == nullptr) {
    ...
    std::tie(std::ignore, err) =
        storage->Entries(li, li, std::numeric_limits<uint64_t>::max());
    if (err == nullptr) {
...
TEST(raft, TestRawNodeRestart) {
  std::vector<raftpb::Entry> entries = {
      ...
      EntryHelper{.index = 2, .term = 1, .data = "foo"}.Done()};
  raftpb::HardState st;
  st.set_term(1);
  st.set_commit(1);
...
//   - Raft does not emit a HardState, but when the app calls Advance(), it
//   bumps
//     its internal applied index cursor to 10 (when it should be 9)
//   - the next Ready asks the app to apply index 11 (omitting index 10), losing
//   a
//     write.
TEST(raft, TestRawNodeCommitPaginationAfterRestart) {
  ...
  // our storage will ignore this and *will* return it (which is how the Commit
  // index ended up being 10 initially).
  ...
  for (size_t highestApplied = 0; highestApplied != 11;) {
    ...
    EXPECT_TRUE(n != 0);
    if (highestApplied != 0 &&
        highestApplied + 1 != rd.committedEntries_[0].index()) {
      EXPECT_FALSE(true);
    }
...
TEST(raft, TestRawNodeConsumeReady) {
  ...
  EXPECT_FALSE(
      rd.messages_.size() != 1 ||
      !google::protobuf::util::MessageDifferencer::Equals(rd.messages_[0], m1));
  ...
  rn->Advance(rd);
  EXPECT_FALSE(rn->raft_->msgs_.size() != 1 ||
