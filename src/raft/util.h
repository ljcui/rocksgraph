...
#ifdef RAFT_LOG
enum class LOG_LEVEL { FATAL = 0, ERROR, WARN, INFO, DEBUG };
const char *const log_level_to_str[] = {"FATAL", "ERROR", "WARN", "INFO",
                                        "DEBUG"};
...
class Error {
 public:
  ...
  explicit Error(std::string msg) : code_(Unknown), msg_(std::move(msg)) {}
  ...
  bool operator!=(nullptr_t) const { return code_ != Ok; }

  bool operator!=(const Error &err) const { return code_ != err.code_; }
  ...
  [[nodiscard]] std::string String() const { return msg_; }
...
inline bool IsLocalMsg(raftpb::MessageType msgt) {
  return msgt == raftpb::MsgHup || msgt == raftpb::MsgBeat ||
         msgt == raftpb::MsgUnreachable || msgt == raftpb::MsgSnapStatus ||
         msgt == raftpb::MsgCheckQuorum || msgt == raftpb::MsgStorageAppend ||
         msgt == raftpb::MsgStorageAppendResp ||
         msgt == raftpb::MsgStorageApply || msgt == raftpb::MsgStorageApplyResp;
}

inline bool IsResponseMsg(raftpb::MessageType msgt) {
  return msgt == raftpb::MsgAppResp || msgt == raftpb::MsgVoteResp ||
         msgt == raftpb::MsgHeartbeatResp || msgt == raftpb::MsgUnreachable ||
         msgt == raftpb::MsgReadIndexResp || msgt == raftpb::MsgPreVoteResp ||
         msgt == raftpb::MsgStorageAppendResp ||
...
inline raftpb::MessageType voteRespMsgType(raftpb::MessageType msgt) {
  switch (msgt) {
    ...
    default:
      ERAFT_FATAL("not a vote message: %s",
                  raftpb::MessageType_Name(msgt).c_str());
      return msgt;
  }
}
...
inline uint64_t payloadSize(const raftpb::Entry &e) { return e.data().size(); }
...
inline bool isHardStateEqual(const raftpb::HardState &a,
                             const raftpb::HardState &b) {
...
template <class T>
inline bool VectorEquals(const std::vector<T> &left,
                         const std::vector<T> &right) {
  if (left.size() != right.size()) {
    return false;
  }
  for (size_t i = 0; i < left.size(); i++) {
    if (!google::protobuf::util::MessageDifferencer::Equals(left[i],
                                                            right[i])) {
      return false;
    }
  }
