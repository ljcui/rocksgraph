...
std::vector<raftpb::Entry> nextEnts(const std::shared_ptr<raft>& r,
                                    const std::shared_ptr<MemoryStorage>& s) {
...
void mustAppendEntry(std::shared_ptr<raft> r,
                     const std::vector<raftpb::Entry>& ents) {
  if (!r->appendEntry(ents)) {
    ERAFT_FATAL("entry unexpectedly dropped");
  }
}

Config newTestConfig(uint64_t id, int election, int heartbeat,
...
std::shared_ptr<raft> newTestRaft(uint64_t id, int election, int heartbeat,
                                  std::shared_ptr<Storage> storage) {
...
using testMemoryStorageOptions =
    std::function<void(std::shared_ptr<MemoryStorage>&)>;

std::shared_ptr<MemoryStorage> newTestMemoryStorage(
    const std::vector<testMemoryStorageOptions>& opts) {
...
std::shared_ptr<raft> newTestLearnerRaft(uint64_t id, int election,
                                         int heartbeat,
                                         std::shared_ptr<Storage> storage) {
...
std::vector<raftpb::Message> takeMessagesAfterAppend(
    const std::shared_ptr<raft>& r) {
  auto msgs = r->msgsAfterAppend_;
  r->msgsAfterAppend_.clear();
  return msgs;
}

Error stepOrSend(const std::shared_ptr<raft>& r,
...
testMemoryStorageOptions withPeers(const std::vector<uint64_t>& peers) {
  return [peers](std::shared_ptr<MemoryStorage>& ms) {
    *ms->snapshot_.mutable_metadata()->mutable_conf_state()->mutable_voters() =
...
testMemoryStorageOptions withLearners(const std::vector<uint64_t>& learners) {
  return [learners](std::shared_ptr<MemoryStorage>& ms) {
    *ms->snapshot_.mutable_metadata()
         ->mutable_conf_state()
         ->mutable_learners() = {learners.begin(), learners.end()};
  };
}
...
TEST(raft, TestProgressLeader) {
  ...
  // Send proposals to r1. The first 5 entries should be queued in the unstable
  // log.
  ...
  if ((ents.size() != 6) || ents[0].data().size() > 0 ||
      ents[5].data() != "foo") {
    EXPECT_TRUE(false);
  }
  advanceMessagesAfterAppend(r);
...
TEST(raft, TestProgressFlowControl) {
  ...
  cfg.maxInflightBytes_ =
      9000;  // A little over MaxInflightMsgs * MaxSizePerMsg.
  auto r = newRaft(cfg);
  r->becomeCandidate();
  r->becomeLeader();
  ...
  EXPECT_FALSE(ms[0].entries(0).data().size() != 0 ||
               ms[0].entries(1).data().size() != 1000);
...
TEST(raft, TestUncommittedEntryLimit) {
  ...
  auto cfg =
      newTestConfig(1, 5, 1, newTestMemoryStorage({withPeers({1, 2, 3})}));
...
void preVoteConfig(Config* c) { c->preVote_ = true; }
...
struct blackHole : public stateMachine {
  Error Step(raftpb::Message) override { return {}; }
  std::vector<raftpb::Message> readMessages() override { return {}; }
...
struct raftWarp : public stateMachine {
  ...
  Error Step(raftpb::Message m) override { return raft_->Step(std::move(m)); }
...
std::shared_ptr<raftWarp> entsWithConfig(
    const std::function<void(Config*)>& configFunc,
...
std::shared_ptr<raftWarp> votedWithConfig(
    const std::function<void(Config*)>& configFunc, uint64_t vote,
    uint64_t term) {
  auto storage = NewMemoryStorage();
  raftpb::HardState hs;
  hs.set_vote(vote);
...
struct network {
  ...
  std::vector<raftpb::Message> filter(
      const std::vector<raftpb::Message>& msgs) {
  ...
  void ignore(raftpb::MessageType t) { ignorem[t] = true; }
...
std::shared_ptr<network> newNetworkWithConfig(
    const std::function<void(Config*)>& configFunc,
    const std::vector<std::shared_ptr<stateMachine>>& peers) {
  ...
  for (size_t j = 0; j < peers.size(); j++) {
    ...
    if (!p) {
      ...
      v->id_ = id;
      v->trk_ = tracker::MakeProgressTracker(v->trk_.maxInflight_,
                                             v->trk_.maxInflightBytes_);
...
std::shared_ptr<network> newNetwork(
    const std::vector<std::shared_ptr<stateMachine>>& peers) {
...
void testLeaderElection(bool preVote) {
  ...
  std::vector<Test> tests = {
      ...
      {newNetworkWithConfig(cfg, {nullptr, nullptr, nopStepper}), StateLeader,
       1},
  ...
  for (auto& tt : tests) {
    ...
    tt.network_->send({m});
    auto sm =
        std::dynamic_pointer_cast<raftWarp>(tt.network_->peers.at(1))->raft_;
    EXPECT_EQ(sm->state_, tt.state);
    EXPECT_EQ(sm->term_, tt.expTerm);
  }
}

TEST(raft, TestLeaderElection) { testLeaderElection(false); }

TEST(raft, TestLeaderElectionPreVote) { testLeaderElection(true); }

// TestLearnerElectionTimeout verfies that the leader should not start election
// even when times out.
TEST(raft, TestLearnerElectionTimeout) {
  auto n1 = newTestLearnerRaft(
      1, 10, 1, newTestMemoryStorage({withPeers({1}), withLearners({2})}));
  auto n2 = newTestLearnerRaft(
      2, 10, 1, newTestMemoryStorage({withPeers({1}), withLearners({2})}));
...
TEST(raft, TestLearnerPromotion) {
  ...
  auto nt = newNetwork(
      {std::make_shared<raftWarp>(n1), std::make_shared<raftWarp>(n2)});
...
void testLeaderCycle(bool preVote) {
  ...
  for (uint64_t campaignerID = 1; campaignerID <= 3; campaignerID++) {
    ...
    for (auto& pair : n->peers) {
      ...
      EXPECT_FALSE(sm->raft_->id_ == campaignerID &&
                   sm->raft_->state_ != StateLeader);
      EXPECT_FALSE(sm->raft_->id_ != campaignerID &&
                   sm->raft_->state_ != StateFollower);
    }
  }
}

TEST(raft, TestLeaderCycle) { testLeaderCycle(false); }

TEST(raft, TestLeaderCyclePreVote) { testLeaderCycle(true); }
...
void testLeaderElectionOverwriteNewerLogs(bool preVote) {
  ...
  auto n = newNetworkWithConfig(
      cfg, {entsWithConfig(cfg, {1}),      // Node 1: Won first election
...
void testVoteFromAnyState(raftpb::MessageType vt) {
  for (auto n = 0; n < (int)numStates; n++) {
    auto r =
        newTestRaft(1, 10, 1, newTestMemoryStorage({withPeers({1, 2, 3})}));
    r->term_ = 1;
    auto st = (StateType)n;
    switch (st) {
      case StateFollower:
...
TEST(raft, TestVoteFromAnyState) { testVoteFromAnyState(raftpb::MsgVote); }
...
TEST(raft, TestLogReplication) {
  ...
  std::vector<Test> tests = {
      {
          ...
          {MessageHelper{.from = 1,
                         .to = 1,
                         .type = raftpb::MsgProp,
                         .entries = {EntryHelper{.data = "somedata"}.Done()}}
               .Done()},
          2,
      },
      {
  ...
  for (auto& tt : tests) {
    tt.net->send(
        {MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgHup}.Done()});
    for (auto& m : tt.msgs) {
      tt.net->send({m});
    }
...
TEST(raft, TestSingleNodeCommit) {
  ...
  tt->send({MessageHelper{.from = 1,
                          .to = 1,
                          .type = raftpb::MsgProp,
                          .entries = {EntryHelper{.data = "some data"}.Done()}}
                .Done()});
  tt->send({MessageHelper{.from = 1,
                          .to = 1,
                          .type = raftpb::MsgProp,
...
TEST(raft, TestDuelingCandidates) {
  ...
  auto nt =
      newNetwork({std::make_shared<raftWarp>(a), std::make_shared<raftWarp>(b),
                  std::make_shared<raftWarp>(c)});
  nt->cut(1, 3);
  ...
  std::vector<Test> tests = {{a, StateFollower, 2, 1},
                             {b, StateFollower, 2, 1},
                             {c, StateFollower, 2, 0}};
...
TEST(raft, TestDuelingPreCandidates) {
  auto cfgA =
      newTestConfig(1, 10, 1, newTestMemoryStorage({withPeers({1, 2, 3})}));
  auto cfgB =
      newTestConfig(2, 10, 1, newTestMemoryStorage({withPeers({1, 2, 3})}));
  auto cfgC =
      newTestConfig(3, 10, 1, newTestMemoryStorage({withPeers({1, 2, 3})}));
  cfgA.preVote_ = true;
  cfgB.preVote_ = true;
  cfgC.preVote_ = true;
  auto a = newRaft(cfgA);
  ...
  std::vector<Test> tests = {{a, StateLeader, 1, 1},
                             {b, StateFollower, 1, 1},
                             {c, StateFollower, 1, 0}};
...
TEST(raft, TestCandidateConcede) {
  ...
  tt->send({MessageHelper{.from = 3,
                          .to = 3,
                          .type = raftpb::MsgProp,
                          .entries = {EntryHelper{.data = data}.Done()}}
                .Done()});
  ...
  ms->ents_ = {{},
               ...
               EntryHelper{.index = 2, .term = 1, .data = data}.Done()};
  wlog->storage_ = ms;
  wlog->committed_ = 2;
...
TEST(raft, TestOldMessages) {
  ...
  tt->send({MessageHelper{.from = 2, .to = 2, .type = raftpb::MsgHup}.Done()});
  tt->send({MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgHup}.Done()});
  // pretend we're an old leader trying to make progress; this entry is expected
  // to be ignored.
  tt->send(
      {MessageHelper{.from = 2,
                     .to = 1,
                     .term = 2,
                     .type = raftpb::MsgApp,
                     .entries = {EntryHelper{.index = 3, .term = 2}.Done()}}
           .Done()});
  // commit a new entry
  tt->send({MessageHelper{.from = 1,
                          .to = 1,
  ...
  ms->ents_ = {{},
               ...
               EntryHelper{.index = 4, .term = 3, .data = "somedata"}.Done()};
  wlog->storage_ = ms;
  wlog->committed_ = 4;
...
TEST(raft, TestProposal) {
  ...
  for (auto& tt : tests) {
    ...
    tt.net->send(
        {MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgHup}.Done()});
    tt.net->send({MessageHelper{.from = 1,
                                .to = 1,
                                .type = raftpb::MsgProp,
                                .entries = {EntryHelper{.data = data}.Done()}}
                      .Done()});
...
TEST(raft, TestPastElectionTimeout) {
  ...
  std::vector<Test> tests = {
      {5, 0, false},   {10, 0.1, true}, {13, 0.4, true},
...
//  terms),
//     delete the existing entry and all that follow it; append any new entries
...
TEST(raft, TestHandleMsgApp) {
  ...
  msg.set_index(0);
  msg.set_commit(1);
  ents = {EntryHelper{.index = 1, .term = 2}.Done()};
  *msg.mutable_entries() = {ents.begin(), ents.end()};
  ...
  msg.set_index(2);
  msg.set_commit(3);
  ents = {EntryHelper{.index = 3, .term = 2}.Done(),
  ...
  *msg.mutable_entries() = {ents.begin(), ents.end()};
  ...
  msg.set_index(1);
  msg.set_commit(4);
  ents = {EntryHelper{.index = 2, .term = 2}.Done()};
  *msg.mutable_entries() = {ents.begin(), ents.end()};
  ...
  *msg.mutable_entries() = {ents.begin(), ents.end()};
  ...
  for (auto& tt : tests) {
    auto storage = newTestMemoryStorage({withPeers({1})});
    storage->Append({EntryHelper{.index = 1, .term = 1}.Done(),
                     EntryHelper{.index = 2, .term = 2}.Done()});
...
TEST(raft, TestMsgAppRespWaitReset) {
  ...
  // Node 2 left the wait state due to its MsgAppResp, but node 3 is still
  // waiting.
  auto msgs = readMessages(sm);
  EXPECT_EQ(msgs.size(), 1);
  EXPECT_FALSE(msgs[0].type() != raftpb::MsgApp || msgs[0].to() != 2);
  EXPECT_FALSE(msgs[0].entries().size() != 1 ||
...
void testRecvMsgVote(raftpb::MessageType msgType) {
  ...
  std::vector<Test> tests = {
      {StateFollower, 0, 0, None, true},  {StateFollower, 0, 1, None, true},
      {StateFollower, 0, 2, None, true},  {StateFollower, 0, 3, None, false},

      {StateFollower, 1, 0, None, true},  {StateFollower, 1, 1, None, true},
      {StateFollower, 1, 2, None, true},  {StateFollower, 1, 3, None, false},

      {StateFollower, 2, 0, None, true},  {StateFollower, 2, 1, None, true},
      {StateFollower, 2, 2, None, false}, {StateFollower, 2, 3, None, false},

      {StateFollower, 3, 0, None, true},  {StateFollower, 3, 1, None, true},
      {StateFollower, 3, 2, None, false}, {StateFollower, 3, 3, None, false},

      {StateFollower, 3, 2, 2, false},    {StateFollower, 3, 2, 1, true},

      {StateLeader, 3, 3, 1, true},       {StatePreCandidate, 3, 3, 1, true},
...
TEST(raft, TestRecvMsgVote) { testRecvMsgVote(raftpb::MsgVote); }

TEST(raft, TestRecvMsgPreVote) { testRecvMsgVote(raftpb::MsgPreVote); }
...
TEST(raft, TestAllServerStepdown) {
  ...
  std::vector<raftpb::MessageType> tmsgTypes = {raftpb::MsgVote,
                                                raftpb::MsgApp};
  uint64_t tterm = 3;
  for (auto& tt : tests) {
    auto sm =
...
void testCandidateResetTerm(raftpb::MessageType mt) {
  ...
  nt->send(
      {MessageHelper{.from = 1, .to = 3, .term = a->term_, .type = mt}.Done()});
...
void testCandidateSelfVoteAfterLostElection(bool preVote) {
  ...
  // Becomes a follower.
  auto m =
      MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgHeartbeat}.Done();
  m.set_term(sm->term_);
  sm->Step(m);
...
// that it has lost the election. The self-vote should be ignored in these
// cases.
...
TEST(raft, TestCandidateDeliversPreCandidateSelfVoteAfterBecomingCandidate) {
  ...
  auto m1 =
      MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgPreVoteResp}.Done();
  m1.set_term(sm->term_ + 1);
  sm->Step(m1);
  auto m2 =
      MessageHelper{.from = 3, .to = 1, .type = raftpb::MsgPreVoteResp}.Done();
  ...
  auto m3 =
      MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgVoteResp}.Done();
...
TEST(raft, TestLeaderElectionWithCheckQuorum) {
  ...
  // because the value might be reset to electionTimeout since the last state
  // changes
  setRandomizedElectionTimeout(a, a->electionTimeout_ + 1);
  setRandomizedElectionTimeout(b, b->electionTimeout_ + 2);
...
TEST(raft, TestFreeStuckCandidateWithCheckQuorum) {
  ...
  auto msg =
      MessageHelper{.from = 1, .to = 3, .type = raftpb::MsgHeartbeat}.Done();
...
TEST(raft, TestNonPromotableVoterWithCheckQuorum) {
  ...
  auto nt = newNetwork(
      {std::make_shared<raftWarp>(a), std::make_shared<raftWarp>(b)});
  setRandomizedElectionTimeout(b, b->electionTimeout_ + 1);
...
TEST(raft, TestDisruptiveFollower) {
  ...
  auto nt = newNetwork({std::make_shared<raftWarp>(n1),
                        std::make_shared<raftWarp>(n2),
                        std::make_shared<raftWarp>(n3)});
  ...
  nt->send({MessageHelper{
      .from = 1, .to = 3, .term = n1->term_, .type = raftpb::MsgHeartbeat}
                .Done()});
...
TEST(raft, TestReadOnlyOptionSafe) {
  ...
  std::vector<Test> tests = {{a, 10, 11, "ctx1"}, {b, 10, 21, "ctx2"},
                             {c, 10, 31, "ctx3"}, {a, 10, 41, "ctx4"},
                             {b, 10, 51, "ctx5"}, {c, 10, 61, "ctx6"}};

  for (auto& tt : tests) {
    for (auto j = 0; j < tt.proposals; j++) {
      nt->send({MessageHelper{
          .from = 1, .to = 1, .type = raftpb::MsgProp, .entries = {{}}}
                    .Done()});
    }

    nt->send({{MessageHelper{.from = tt.sm->id_,
                             .to = tt.sm->id_,
                             .type = raftpb::MsgReadIndex,
                             .entries = {EntryHelper{.data = tt.wctx}.Done()}}
                   .Done()}});
...
TEST(raft, TestReadOnlyWithLearner) {
  ...
  std::vector<Test> tests = {{a, 10, 11, "ctx1"},
                             {b, 10, 21, "ctx2"},
                             {a, 10, 31, "ctx3"},
                             {b, 10, 41, "ctx4"}};
  for (auto& tt : tests) {
...
TEST(raft, TestReadOnlyOptionLease) {
  ...
  std::vector<Test> tests = {
      {a, 10, 11, "ctx1"}, {b, 10, 21, "ctx2"}, {c, 10, 31, "ctx3"},
      {a, 10, 41, "ctx4"}, {b, 10, 51, "ctx5"}, {c, 10, 61, "ctx6"},
  };
  for (auto& tt : tests) {
    for (auto j = 0; j < tt.proposals; j++) {
      nt->send({MessageHelper{
...
TEST(raft, TestReadOnlyForNewLeader) {
  ...
  nt->send({MessageHelper{.from = 1,
                          .to = 1,
                          .type = raftpb::MsgReadIndex,
                          .entries = {{EntryHelper{.data = wctx}.Done()}}}
                .Done()});
  ...
  nt->send({MessageHelper{
      .from = 1, .to = 1, .type = raftpb::MsgProp, .entries = {{}}}
                .Done()});
  EXPECT_EQ(sm->raft_->raftLog_->committed_, 4);
  auto lastLogTerm = sm->raft_->raftLog_->zeroTermOnOutOfBounds(
      sm->raft_->raftLog_->term(sm->raft_->raftLog_->committed_));
  ...
  // Ensure peer a accepts read only request after it committed an entry at its
  // term.
  nt->send({MessageHelper{.from = 1,
                          .to = 1,
                          .type = raftpb::MsgReadIndex,
...
TEST(raft, TestLeaderAppResp) {
  ...
  std::vector<Test> tests = {
      ...
      {2, true, 0, 2, 1, 1, 0},  // denied resp; leader does not commit;
                                 // decrease next and send probing msg
      {2, false, 2, 4, 2, 2,
       2},  // accept resp; leader commits; broadcast with commit index
...
TEST(raft, TestBcastBeat) {
  ...
  *meta->mutable_conf_state()->mutable_voters() = {voters.begin(),
                                                   voters.end()};
...
TEST(raft, TestRecvMsgBeat) {
  ...
  for (auto& tt : tests) {
    ...
    sm->Step(
        {MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgBeat}.Done()});
...
TEST(raft, TestSendAppendForProgressProbe) {
  ...
  r->Step({MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgHeartbeatResp}
               .Done()});
  auto msg = readMessages(r);
  EXPECT_EQ(msg.size(), 1);
  EXPECT_EQ(msg[0].index(), 0);
...
TEST(raft, TestRecvMsgUnreachable) {
  ...
  r->Step({MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgUnreachable}
               .Done()});
...
TEST(raft, TestRestore) {
  ...
  EXPECT_EQ(mustTerm(sm->raftLog_->term(s.metadata().index())),
            s.metadata().term());
...
TEST(raft, TestRestoreWithLearner) {
  ...
  meta->set_term(11);   // magic number
  {
    ...
    *meta->mutable_conf_state()->mutable_learners() = {learners.begin(),
                                                       learners.end()};
  }
...
// TestRestoreWithVotersOutgoing tests if outgoing voter can receive and apply
// snapshot correctly.
TEST(raft, TestRestoreWithVotersOutgoing) {
  ...
  meta->set_term(11);   // magic number
  {
    ...
    *meta->mutable_conf_state()->mutable_voters_outgoing() = {
        voters_outgoing.begin(), voters_outgoing.end()};
  }
...
// TestLearnerReceiveSnapshot tests that a learner can receive a snpahost from
// leader
...
TEST(raft, TestSlowNodeRestore) {
  ...
  *cs.mutable_voters() = {vn.begin(), vn.end()};
  ...
  nt->send({MessageHelper{
      .from = 1, .to = 1, .type = raftpb::MsgProp, .entries = {{}}}
                .Done()});
  EXPECT_EQ(follower->raft_->raftLog_->committed_,
            lead->raft_->raftLog_->committed_);
}
...
TEST(raft, TestStepConfig) {
  ...
  r->Step({MessageHelper{
      .from = 1,
      .to = 1,
      .type = raftpb::MsgProp,
      .entries = {EntryHelper{.type = raftpb::EntryConfChange}.Done()}}
               .Done()});
...
TEST(raft, TestCampaignWhileLeader) { testCampaignWhileLeader(false); }

TEST(raft, TestPreCampaignWhileLeader) { testCampaignWhileLeader(true); }
...
TEST(raft, TestCommitAfterRemoveNode) {
  ...
  EXPECT_FALSE(ents[0].type() != raftpb::EntryNormal ||
               !ents[0].data().empty());
  ...
  r->applyConfChange(ccw.AsV2());
  ents = nextEnts(r, s);
  EXPECT_FALSE(ents.size() != 1 || ents[0].type() != raftpb::EntryNormal ||
               ents[0].data() != "hello");
}

void checkLeaderTransferState(const std::shared_ptr<raft>& r, StateType state,
                              uint64_t lead) {
...
TEST(raft, TestLeaderTransferToUpToDateNode) {
  ...
  nt->send({MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgTransferLeader}
                .Done()});
  ...
  nt->send({MessageHelper{
      .from = 1, .to = 1, .type = raftpb::MsgProp, .entries = {{}}}
                .Done()});
  nt->send({MessageHelper{.from = 1, .to = 2, .type = raftpb::MsgTransferLeader}
                .Done()});
...
TEST(raft, TestLeaderTransferToUpToDateNodeFromFollower) {
  ...
  // Transfer leadership to 2.
  nt->send({MessageHelper{.from = 2, .to = 2, .type = raftpb::MsgTransferLeader}
                .Done()});
  ...
  nt->send({MessageHelper{
      .from = 1, .to = 1, .type = raftpb::MsgProp, .entries = {{}}}
                .Done()});
  nt->send({MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgTransferLeader}
                .Done()});
...
TEST(raft, TestLeaderTransferToSlowFollower) {
  ...
  nt->send({MessageHelper{.from = 3, .to = 1, .type = raftpb::MsgTransferLeader}
                .Done()});
...
TEST(raft, TestLeaderTransferAfterSnapshot) {
  ...
  *cs.mutable_voters() = {vn.begin(), vn.end()};
  ...
  EXPECT_FALSE(google::protobuf::util::MessageDifferencer::Equals(
      filtered, raftpb::Message()));
...
TEST(raft, TestLeaderTransferToNonExistingNode) {
  ...
  nt->send({MessageHelper{.from = 4, .to = 1, .type = raftpb::MsgTransferLeader}
                .Done()});
  checkLeaderTransferState(lead->raft_, StateLeader, 1);
}
...
TEST(raft, TestLeaderTransferTimeout) {
  ...
  for (auto i = 0;
       i < lead->raft_->electionTimeout_ - lead->raft_->heartbeatTimeout_;
       i++) {
    lead->raft_->tick_();
  }
...
TEST(raft, TestLeaderTransferIgnoreProposal) {
  ...
  nextEnts(r, s);  // handle empty entry
                   // Transfer leadership to isolated node to let transfer
                   // pending, then send proposal.
  nt->send({MessageHelper{.from = 3, .to = 1, .type = raftpb::MsgTransferLeader}
                .Done()});
  EXPECT_EQ(lead->raft_->leadTransferee_, 3);
  ...
  auto err = lead->raft_->Step({MessageHelper{
      .from = 1, .to = 1, .type = raftpb::MsgProp, .entries = {{}}}
                                    .Done()});
...
TEST(raft, TestTransferNonMember) {
  ...
  r->Step({MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgTimeoutNow}
               .Done()});

  r->Step(
      {MessageHelper{.from = 2, .to = 1, .type = raftpb::MsgVoteResp}.Done()});
  r->Step(
      {MessageHelper{.from = 3, .to = 1, .type = raftpb::MsgVoteResp}.Done()});
  EXPECT_EQ(r->state_, StateFollower);
}
...
TEST(raft, TestNodeWithSmallerTermCanCompleteElection) {
  ...
  EXPECT_FALSE(sma->raft_->state_ != StateLeader &&
               smb->raft_->state_ != StateLeader);
}
...
TEST(raft, TestPreVoteWithSplitVote) {
  ...
  nt->send({MessageHelper{.from = 2, .to = 2, .type = raftpb::MsgHup}.Done(),
            MessageHelper{.from = 3, .to = 3, .type = raftpb::MsgHup}.Done()});
...
TEST(raft, TestLearnerCampaign) {
  ...
  nt->send({MessageHelper{.from = 1, .to = 2, .type = raftpb::MsgTimeoutNow}
                .Done()});
  EXPECT_EQ(n2->state_, StateFollower);
}
...
TEST(raft, TestPreVoteMigrationCanCompleteElection) {
  ...
  // Do we have a leader?
  EXPECT_FALSE(n2->raft_->state_ != StateLeader &&
               n3->raft_->state_ != StateFollower);
}
...
// TestConfChangeCheckBeforeCampaign tests if unapplied ConfChange is checked
// before campaign.
TEST(raft, TestConfChangeCheckBeforeCampaign) {
  testConfChangeCheckBeforeCampaign(false);
}

// TestConfChangeV2CheckBeforeCampaign tests if unapplied ConfChangeV2 is
// checked before campaign.
...
TEST(raft, TestFastLogRejection) {
  struct Test {
    ...
    uint64_t rejectHintTerm =
        0;  // Expected term included in rejected MsgAppResp.
    uint64_t rejectHintIndex =
        0;  // Expected index included in rejected MsgAppResp.
    uint64_t nextAppendTerm =
        0;  // Expected term when leader appends after rejected.
    uint64_t nextAppendIndex =
        0;  // Expected index when leader appends after rejected.
    uint64_t followerCompact =
        0;  // Index at which the follower log is compacted.
  };
  std::vector<Test> tests = {
      ...
      // delivered to a follower who has already compacted beyond log index 3.
      // The
      // MsgAppResp rejection will return same index=3, with logTerm=0. The
      // leader
      ...
      {{EntryHelper{.index = 1, .term = 1}.Done(),
        ...
        EntryHelper{.index = 3, .term = 3}.Done()},
       {
           ...
           EntryHelper{.index = 3, .term = 3}.Done(),
           EntryHelper{.index = 4, .term = 3}.Done(),
           EntryHelper{.index = 5, .term = 3}
               .Done()  // <- this entry and below are compacted
       },
       0,
       3,
       1,
  ...
  for (auto& test : tests) {
    ...
    std::vector<uint64_t> voters = {1, 2, 3};
    *s1->snapshot_.mutable_metadata()->mutable_conf_state()->mutable_voters() =
    ...
    *s2->snapshot_.mutable_metadata()->mutable_conf_state()->mutable_voters() =
        {voters.begin(), voters.end()};
    ...
    EXPECT_EQ(
        n2->Step(MessageHelper{.from = 1, .to = 2, .type = raftpb::MsgHeartbeat}
                     .Done()),
        nullptr);
    auto msgs = readMessages(n2);
    EXPECT_EQ(msgs.size(), 1);
...
TEST(raft, TestLeaderSyncFollowerLog) {
  std::vector<raftpb::Entry> ents = {
      ...
      EntryHelper{.index = 6, .term = 5}.Done(),
      EntryHelper{.index = 7, .term = 5}.Done(),
      EntryHelper{.index = 8, .term = 6}.Done(),
      EntryHelper{.index = 9, .term = 6}.Done(),
      EntryHelper{.index = 10, .term = 6}.Done(),
  };
  uint64_t term = 8;
  std::vector<std::vector<raftpb::Entry>> tests = {
      ...
      },
      {
          ...
          EntryHelper{.index = 9, .term = 6}.Done(),
          EntryHelper{.index = 10, .term = 6}.Done(),
          EntryHelper{.index = 11, .term = 6}.Done(),
      },
      {
          {},
          EntryHelper{.index = 1, .term = 1}.Done(),
          ...
          EntryHelper{.index = 9, .term = 6}.Done(),
          EntryHelper{.index = 10, .term = 6}.Done(),
          EntryHelper{.index = 11, .term = 7}.Done(),
          EntryHelper{.index = 12, .term = 7}.Done(),
      },
      {
          {},
          EntryHelper{.index = 1, .term = 1}.Done(),
  ...
  for (auto& tt : tests) {
    ...
    auto n = newNetwork({std::make_shared<raftWarp>(lead),
                         std::make_shared<raftWarp>(follower), nopStepper});
    n->send({MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgHup}.Done()});
    // The election occurs in the term after the one we loaded with
    // lead.loadState above.
    n->send({MessageHelper{
        .from = 3, .to = 1, .term = term + 1, .type = raftpb::MsgVoteResp}
                 .Done()});

    auto msg =
        MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgProp}.Done();
    msg.mutable_entries()->Add();
    n->send({msg});
