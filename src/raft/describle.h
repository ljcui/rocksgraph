...
inline std::string DescribeConfState(const raftpb::ConfState& state) {
  return format(
      "Voters:[%s] VotersOutgoing:[%s] Learners:[%s] LearnersNext:[%s] "
      "AutoLeave:%s",
...
inline std::string DescribeSnapshot(const raftpb::Snapshot& snap) {
  ...
  return format("Index:%d Term:%d ConfState:%s", m.index(), m.term(),
                DescribeConfState(m.conf_state()).c_str());
}
...
inline std::string DescribeEntry(const raftpb::Entry& e, EntryFormatter f) {
  if (f == nullptr) {
    f = [](const std::string& data) {
      return format("\"%s\"", ToHex(data).c_str());
    };
  }
  ...
  return format("%d/%d %s%s", e.term(), e.index(),
                raftpb::EntryType_Name(e.type()).c_str(), formatted.c_str());
}

// DescribeMessage returns a concise human-readable description of a
// Message for debugging.
inline std::string DescribeMessage(const raftpb::Message& m,
                                   const EntryFormatter& f) {
  std::string buf;
  buf.append(format("%s->%s %v Term:%d Log:%d/%d",
                    describeTarget(m.from()).c_str(),
                    describeTarget(m.to()).c_str(), m.type(), m.term(),
                    m.logterm(), m.index()));
  if (m.reject()) {
