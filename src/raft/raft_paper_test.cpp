...
extern std::shared_ptr<MemoryStorage> newTestMemoryStorage(
    const std::vector<testMemoryStorageOptions>& opts);
extern testMemoryStorageOptions withPeers(const std::vector<uint64_t>& peers);
extern std::shared_ptr<raft> newTestRaft(uint64_t id, int election,
                                         int heartbeat,
                                         std::shared_ptr<Storage> storage);
extern std::vector<raftpb::Message> readMessages(
    const std::shared_ptr<raft>& r);
extern void mustAppendEntry(std::shared_ptr<raft> r,
...
struct network;
struct stateMachine;
extern std::shared_ptr<network> newNetwork(
...
TEST(raft, TestLeaderBcastBeat) {
  ...
  std::sort(msgs.begin(), msgs.end(),
            [](const raftpb::Message& l, const raftpb::Message& r) {
              return l.ShortDebugString() < r.ShortDebugString();
            });

  std::vector<raftpb::Message> wmsgs = {
      MessageHelper{.from = 1, .to = 2, .term = 1, .type = raftpb::MsgHeartbeat}
          .Done(),
      MessageHelper{.from = 1, .to = 3, .term = 1, .type = raftpb::MsgHeartbeat}
          .Done()};
...
void testNonleaderStartElection(StateType state) {
  ...
  std::vector<raftpb::Message> wmsgs = {
      MessageHelper{.from = 1, .to = 2, .term = 2, .type = raftpb::MsgVote}
          .Done(),
      MessageHelper{.from = 1, .to = 3, .term = 2, .type = raftpb::MsgVote}
          .Done()};
  EXPECT_TRUE(VectorEquals(msgs, wmsgs));
}
...
TEST(raft, TestLeaderElectionInOneRoundRPC) {
  ...
  for (auto& tt : tests) {
    auto r = newTestRaft(1, 10, 1,
                         newTestMemoryStorage({withPeers(idsBySize(tt.size))}));
    r->Step(MessageHelper{.from = 1, .to = 1, .type = raftpb::MsgHup}.Done());
    advanceMessagesAfterAppend(r);
    for (auto& pair : tt.votes) {
      auto m = MessageHelper{.from = pair.first,
                             .to = 1,
                             .term = r->term_,
...
TEST(raft, TestFollowerVote) {
  ...
  std::vector<Test> tests = {
      {None, 2, false}, {None, 3, false}, {2, 2, false},
      {3, 3, false},    {2, 3, true},     {3, 2, true},
  };
  for (auto& tt : tests) {
    auto r =
    ...
    r->Step(MessageHelper{
        .from = tt.nvote, .to = 1, .term = 1, .type = raftpb::MsgVote}
                .Done());

    auto msgs = r->msgsAfterAppend_;
    auto m =
        MessageHelper{.from = 1, .to = tt.nvote, .type = raftpb::MsgVoteResp}
            .Done();
    m.set_term(1);
...
TEST(raft, TestCandidateFallback) {
  std::vector<raftpb::Message> tests = {
      MessageHelper{.from = 2, .to = 1, .term = 1, .type = raftpb::MsgApp}
          .Done(),
      MessageHelper{.from = 2, .to = 1, .term = 2, .type = raftpb::MsgApp}
          .Done()};
  for (auto& tt : tests) {
    auto r =
        newTestRaft(1, 10, 1, newTestMemoryStorage({withPeers({1, 2, 3})}));
...
void commitNoopEntry(std::shared_ptr<raft> r,
                     std::shared_ptr<MemoryStorage> s) {
  EXPECT_EQ(r->state_, StateLeader);
  r->bcastAppend();
  // simulate the response of MsgApp
  auto msgs = readMessages(r);
  for (auto& m : msgs) {
    EXPECT_FALSE(m.type() != raftpb::MsgApp || m.entries().size() != 1 ||
...
TEST(raft, TestLeaderStartReplication) {
  ...
  r->Step(MessageHelper{
      .from = 1, .to = 1, .type = raftpb::MsgProp, .entries = ents}
              .Done());
  ...
  std::vector<raftpb::Entry> wents = {
      EntryHelper{.index = li + 1, .term = 1, .data = "some data"}.Done()};
  std::vector<raftpb::Message> wmsgs;
  {
    raftpb::Message tmp;
    tmp.set_from(1);
...
TEST(raft, TestLeaderAcknowledgeCommit) {
  ...
  for (auto& tt : tests) {
    ...
    auto msgs = r->msgs_;
    for (auto& m : msgs) {
      if (tt.nonLeaderAcceptors.count(m.to()) &&
          tt.nonLeaderAcceptors.at(m.to())) {
        r->Step(acceptAndReply(m));
      }
    }
...
TEST(raft, TestLeaderCommitPrecedingEntries) {
  ...
  for (auto& tt : tests) {
    ...
    wents.push_back(
        EntryHelper{.index = li + 2, .term = 3, .data = "some data"}.Done());
    auto g = r->raftLog_->nextCommittedEnts(true);
    EXPECT_TRUE(VectorEquals(g, wents));
  }
}
...
TEST(raft, TestFollowerCommitEntry) {
  ...
  std::vector<Test> tests = {
      ...
      {
          {EntryHelper{.index = 1, .term = 1, .data = "some data"}.Done(),
           EntryHelper{.index = 2, .term = 1, .data = "some data"}.Done()},
          2,
      },
      {
          {EntryHelper{.index = 1, .term = 1, .data = "some data2"}.Done(),
           EntryHelper{.index = 2, .term = 1, .data = "some data"}.Done()},
          2,
      },
      {
          {EntryHelper{.index = 1, .term = 1, .data = "some data"}.Done(),
           EntryHelper{.index = 2, .term = 1, .data = "some data2"}.Done()},
          1,
      }};
  for (auto& tt : tests) {
    auto r =
    ...
    *msg.mutable_entries() = {tt.ents.begin(), tt.ents.end()};
    ...
    std::vector<raftpb::Entry> wents{tt.ents.begin(),
                                     tt.ents.begin() + tt.commit};
    auto g = r->raftLog_->nextCommittedEnts(true);
    EXPECT_TRUE(VectorEquals(g, wents));
  }
}
...
// entry in its log with the same index and term as the one in AppendEntries
// RPC, then it refuses the new entries. Otherwise it replies that it accepts
// the append entries. Reference: section 5.3
TEST(raft, TestFollowerCheckMsgApp) {
  std::vector<raftpb::Entry> ents = {EntryHelper{.index = 1, .term = 1}.Done(),
                                     EntryHelper{.index = 2, .term = 2}.Done()};
  struct Test {
    uint64_t term;
    uint64_t index;
...
TEST(raft, TestFollowerAppendEntries) {
  ...
  std::vector<Test> tests = {{2,
                              ...
                              {EntryHelper{.index = 3, .term = 3}.Done()}},
                             {
                                 1,
                                 1,
                                 {EntryHelper{.index = 2, .term = 3}.Done(),
...
TEST(raft, TestLeaderOnlyCommitsLogFromCurrentTerm) {
  ...
  for (auto& tt : tests) {
    ...
    auto msg =
        MessageHelper{
            .from = 2, .to = 1, .term = r->term_, .type = raftpb::MsgAppResp}
            .Done();
    msg.set_index(tt.index);
    r->Step(msg);
    advanceMessagesAfterAppend(r);
