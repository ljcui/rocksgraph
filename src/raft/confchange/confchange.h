...
namespace detail {
inline const quorum::MajorityConfig& incoming(
    const quorum::JointConfig& voters) {
  return voters[0];
}

inline quorum::MajorityConfig& incoming(quorum::JointConfig& voters) {
  return voters[0];
}

inline const quorum::MajorityConfig& outgoing(
    const quorum::JointConfig& voters) {
  return voters[1];
}

inline quorum::MajorityConfig& outgoing(quorum::JointConfig& voters) {
  return voters[1];
}
}  // namespace detail
...
inline std::string Describe(const std::vector<raftpb::ConfChangeSingle>& ccs) {
  ...
  for (auto& cc : ccs) {
    ...
    buf.append(format("%s(%d)", raftpb::ConfChangeType_Name(cc.type()).c_str(),
                      cc.node_id()));
  }
  return buf;
}
...
inline size_t symdiff(const std::unordered_set<uint64_t>& l,
                      const std::unordered_set<uint64_t>& r) {
  size_t n = 0;
  std::vector<std::vector<quorum::MajorityConfig>> pairs{
      {quorum::MajorityConfig(l),
       quorum::MajorityConfig(r)},  // count elems in l but not in r
      {quorum::MajorityConfig(r),
       quorum::MajorityConfig(l)}  // count elems in r but not in l
  };
  for (auto& p : pairs) {
...
inline Error checkInvariants(const tracker::Config& cfg,
                             const tracker::ProgressMap& trk) {
  ...
  // config.
  for (auto& ids : std::vector<std::unordered_set<uint64_t>>{
           cfg.voters_.IDs(), cfg.learners_, cfg.learnersNext_}) {
    for (auto id : ids) {
  ...
  for (auto id : cfg.learnersNext_) {
    ...
    if (trk[id]->isLearner_) {
      return Error(format(
          "%d is in LearnersNext, but is already marked as learner", id));
    }
  }
  ...
  for (auto id : cfg.learners_) {
    ...
    if (!trk[id]->isLearner_) {
      return Error(
          format("%d is in Learners, but is not marked as learner", id));
    }
  }
...
inline std::tuple<tracker::Config, tracker::ProgressMap, Error> checkAndReturn(
    const tracker::Config& cfg, const tracker::ProgressMap& trk) {
  auto err = checkInvariants(cfg, trk);
  if (!err.ok()) {
    return std::make_tuple(tracker::Config{}, tracker::ProgressMap{},
                           std::move(err));
  }
...
struct Changer {
  ...
  Changer(tracker::ProgressTracker t, uint64_t i)
      : tracker(std::move(t)), lastIndex(i) {}
  ...
  std::tuple<tracker::Config, tracker::ProgressMap, Error> checkAndCopy() {
  ...
  std::tuple<tracker::Config, tracker::ProgressMap, Error> err(
      const Error& err) {
    return std::make_tuple(tracker::Config{}, tracker::ProgressMap{}, err);
  }

  // initProgress initializes a new progress for the given node or learner.
  void initProgress(tracker::Config& cfg, tracker::ProgressMap& trk,
                    uint64_t id, bool isLearner) const {
    if (!isLearner) {
    ...
    pro->match_ = 0;
    pro->inflights_ =
        tracker::NewInflights(tracker.maxInflight_, tracker.maxInflightBytes_);
  ...
  // majority config.
  void makeVoter(tracker::Config& cfg, tracker::ProgressMap& trk,
                 uint64_t id) const {
    if (!trk.data().count(id)) {
  ...
  // LeaveJoint().
  void makeLearner(tracker::Config& cfg, tracker::ProgressMap& trk,
                   uint64_t id) {
    if (!trk.data().count(id)) {
  ...
  // state.
  Error apply(tracker::Config& cfg, tracker::ProgressMap& trk,
  ...
  std::tuple<tracker::Config, tracker::ProgressMap, Error> EnterJoint(
      bool autoLeave, const std::vector<raftpb::ConfChangeSingle>& ccs) {
    tracker::Config cfg;
    tracker::ProgressMap trk;
    Error err;
  ...
  //
  // [1]: https://github.com/ongardie/dissertation/blob/master/online-trim.pdf
  std::tuple<tracker::Config, tracker::ProgressMap, Error> LeaveJoint() {
    tracker::Config cfg;
    tracker::ProgressMap trk;
    Error err;
    std::tie(cfg, trk, err) = checkAndCopy();
  ...
  std::tuple<tracker::Config, tracker::ProgressMap, Error> Simple(
      const std::vector<raftpb::ConfChangeSingle>& ccs) {
    tracker::Config cfg;
    tracker::ProgressMap trk;
    Error err;
    ...
    auto n = detail::symdiff(detail::incoming(tracker.config_.voters_).data(),
                             detail::incoming(cfg.voters_).data());
    if (n > 1) {
      return std::make_tuple(
          tracker::Config{}, tracker::ProgressMap{},
