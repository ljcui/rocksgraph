...
inline std::pair<std::vector<raftpb::ConfChangeSingle>,
                 std::vector<raftpb::ConfChangeSingle>>
...
inline std::tuple<tracker::Config, tracker::ProgressMap, Error> chain(
    Changer chg,
    const std::vector<std::function<
        std::tuple<tracker::Config, tracker::ProgressMap, Error>(Changer)>>&
        ops) {
  for (auto& op : ops) {
...
inline std::tuple<tracker::Config, tracker::ProgressMap, Error> Restore(
  ...
  if (outgoing.empty()) {
    ...
    for (auto& cc : incoming) {
      ops.emplace_back([cc](Changer chg) { return chg.Simple({cc}); });
    }
  } else {
