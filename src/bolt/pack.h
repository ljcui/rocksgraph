...
class Packer {
 public:
  ...
  const std::optional<std::string>& Err() { return err_; }
  ...
  void Int32(int32_t i) { Int64(int64_t(i)); }
  void Int16(int16_t i) { Int64(int64_t(i)); }
  void Int8(int8_t i) { Int64(int64_t(i)); }
  void Int(int i) { Int64(int64_t(i)); }
  void Uint64(uint64_t i) {
    checkOverflowInt(i);
    Int64(int64_t(i));
  }
  void Uint32(uint32_t i) { Int64(int64_t(i)); }
  void Uint16(uint16_t i) { Int64(int64_t(i)); }
  void Uint8(uint8_t i) { Int64(int64_t(i)); }
  ...
  void Float(float f) { Double(double(f)); }
  ...
  void Bools(const std::vector<bool>& ii) {
    ListHeader(ii.size(), 0x90, 0xd4);
    for (auto i : ii) {
      Bool(i);
    }
  }
  void ListHeader(int l) { ListHeader(l, 0x90, 0xd4); }
  void MapHeader(int l) { ListHeader(l, 0xa0, 0xd8); }
  ...
  void Null() { buf_->push_back((uint8_t)0xc0); }
...
struct Unpacker {
 public:
  ...
  int64_t Int();
  double Double();
  uint8_t StructTag() { return Pop(); }
  std::string String();
  bool Bool();
  std::string ByteArray() { return Read(Len()); }
  const std::optional<std::string>& Err() { return err_; }
  PackType CurrentType() { return curr_; }
