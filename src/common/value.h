...
struct Value {
  ...
  [[nodiscard]] bool AsBool() const {
    if (LIKELY(type == ValueType::BOOL)) return std::any_cast<bool>(data);
    THROW_CODE(UnknownError, "AsBool, but Value is not BOOL");
  }
  [[nodiscard]] int64_t AsInteger() const {
    if (LIKELY(type == ValueType::INTEGER)) return std::any_cast<int64_t>(data);
    THROW_CODE(UnknownError, "AsInteger, but Value is not Integer");
  }
  [[nodiscard]] double AsDouble() const {
    if (LIKELY(type == ValueType::DOUBLE)) return std::any_cast<double>(data);
    THROW_CODE(UnknownError, "AsDouble, but Value is not DOUBLE");
  }
  [[nodiscard]] float AsFloat() const {
    if (LIKELY(type == ValueType::FLOAT)) return std::any_cast<float>(data);
  ...
  [[nodiscard]] const std::unordered_map<std::string, Value>& AsMap() const {
  ...
  static Value StringArray(std::vector<std::string> b) {
    return Value(std::move(b));
  }
  static Value Array(std::vector<Value> b) { return Value(std::move(b)); }
  static Value Map(std::unordered_map<std::string, Value> b) {
    return Value(std::move(b));
  }
  static Value Date(const class common::Date& b) { return Value(b); }
  static Value LocalDateTime(const class common::LocalDateTime& b) {
    return Value(b);
  }
  ...
  [[nodiscard]] bool IsScalar() const {
    return type == ValueType::Null || type == ValueType::BOOL ||
           type == ValueType::INTEGER || type == ValueType::STRING ||
           type == ValueType::DOUBLE || type == ValueType::FLOAT;
  }
  ...
  [[nodiscard]] bool IsLocalDateTime() const {
    return type == ValueType::LOCALDATETIME;
  }
  [[nodiscard]] bool IsDateTime() const { return type == ValueType::DATETIME; }
  [[nodiscard]] bool IsLocalTime() const {
    return type == ValueType::LOCALTIME;
  }
...
struct fmt::formatter<std::unordered_map<std::string, Value>> {
  ...
  template <typename FormatContext>
  auto format(const std::unordered_map<std::string, Value>& map,
              FormatContext& ctx) {
    auto out = ctx.out();
    out = fmt::format_to(out, "{{");
