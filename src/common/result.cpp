...
std::string Node::ToString() const {
  return fmt::format("{{id:{}, labels:{}, properties:{}}}", id, labels,
                     properties);
}
...
bolt::InternalPath Path::ToBolt() const {
  ...
  for (size_t i = 0; i < data.size(); i++) {
    ...
    if (p.is_node) {
      ...
      path.rels.push_back(
          std::any_cast<const common::Relationship&>(p.data).ToBoltUnbound());
    }
    if (i >= 1) {
      if (i % 2 == 1) {
        if (std::any_cast<const common::Relationship&>(p.data).src ==
            path.nodes.back().id) {
