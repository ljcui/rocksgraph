...
Time::Time() {
  ...
  nanoseconds_since_today_with_of_ =
...
Time::Time(const std::string& str, std::optional<int64_t> days) {
  ...
  try {
    ...
    if (match[TIME_ZONE_NAME].matched) {
      ...
      if (!days.has_value()) {
        ...
        tz_offset_seconds_ =
            date::zoned_time(
                tmp,
                date::local_time<std::chrono::nanoseconds>(
                    std::chrono::nanoseconds(NANOS_PER_SECOND *
                                             SECONDS_PER_DAY * days.value())))
                .get_info()
                .offset.count();
      }
    }
    std::chrono::nanoseconds ns{hour * 60 * 60 * 1000000000 +
                                minute * 60 * 1000000000 + second * 1000000000 +
                                nanoseconds};
    ...
    THROW_CODE(InputError, "Failed to parse {} into Date, exception: {}", str,
               e.what());
  }
}
...
Time::Time(const Value& params, int64_t truncate) {
  ...
  if (params.IsTime()) {
    nanoseconds_since_today_with_of_ =
        std::get<0>(params.AsTime().GetStorage());
  ...
  if (params.IsDateTime()) {
    nanoseconds_since_today_with_of_ =
        std::get<0>(params.AsDateTime().GetStorage()) %
  ...
  int64_t hour = 0, minute = 0, second = 0, millisecond = 0, microsecond = 0,
          nanosecond = 0, time_param_offset_second = 0;
  ...
  if (parse_params_map.count("time")) {
    ...
    if (parse_params_map["time"].IsLocalTime()) {
      ...
      if (parse_params_map.count("days_for_timezone") &&
          timezone_name_[0] != '+' && timezone_name_[0] != '-') {
        tz_offset_seconds_ = time_param_offset_second =
            date::zoned_time(
                timezone_name_,
                date::local_time<std::chrono::nanoseconds>(
                    std::chrono::nanoseconds(
                        NANOS_PER_SECOND * SECONDS_PER_DAY *
                        parse_params_map["days_for_timezone"].AsInteger())))
                .get_info()
                .offset.count();
      } else {
        tz_offset_seconds_ = time_param_offset_second =
            std::get<1>(parse_params_map["time"].AsTime().GetStorage());
      }
      ...
      if (parse_params_map.count("days_for_timezone") &&
        ...
        tz_offset_seconds_ = time_param_offset_second =
            std::get<1>(parse_params_map["time"].AsDateTime().GetStorage());
      }
    } else {
      THROW_CODE(InvalidParameter, "Cannot get the time of: {}",
                 parse_params_map["time"].ToString());
    }
    nanosecond = v % 1000;
    ...
    std::vector<std::pair<std::string, bool>> has_value = {
        ...
        {"subsecond", parse_params_map.count("millisecond") ||
                          parse_params_map.count("microsecond") ||
                          parse_params_map.count("nanosecond")}};
    ...
    for (const auto& value : has_value) {
      if (!value.second) {
        ...
        THROW_CODE(InvalidParameter, "{} cannot be specified without {}",
                   value.first, firstNotAssigned);
      }
    }
  }
  ...
  auto millisecondValue = parse_params_map["millisecond"],
       microsecondValue = parse_params_map["microsecond"],
       nanosecondValue = parse_params_map["nanosecond"];
  if (!millisecondValue.IsNull() && (millisecondValue.AsInteger() < 0 ||
                                     millisecondValue.AsInteger() >= 1000)) {
    THROW_CODE(InvalidParameter, "Invalid value for Millisecond: {}",
               millisecondValue.AsInteger());
  }
  if (!microsecondValue.IsNull() &&
      (microsecondValue.AsInteger() < 0 ||
       microsecondValue.AsInteger() >=
           (!millisecondValue.IsNull() ? 1000 : 1000000))) {
    THROW_CODE(InvalidParameter, "Invalid value for Microsecond: {}",
               microsecondValue.AsInteger());
  }
  if (!nanosecondValue.IsNull() &&
      (nanosecondValue.AsInteger() < 0 ||
       nanosecondValue.AsInteger() >= (!microsecondValue.IsNull() ? 1000
                                       : !millisecondValue.IsNull()
                                           ? 1000000
                                           : 1000000000))) {
    THROW_CODE(InvalidParameter, "Invalid value for Nanosecond: {}",
               nanosecondValue.AsInteger());
  }
  int64_t sub_second =
      (millisecondValue.IsNull() ? 0 : millisecondValue.AsInteger()) * 1000000 +
      (microsecondValue.IsNull() ? 0 : microsecondValue.AsInteger()) * 1000 +
      (nanosecondValue.IsNull() ? 0 : nanosecondValue.AsInteger());
  if (truncate && sub_second >= truncate &&
      (!millisecondValue.IsNull() || !microsecondValue.IsNull() ||
       !nanosecondValue.IsNull())) {
  ...
  std::chrono::nanoseconds ns{hour * 60 * 60 * 1000000000 +
                              ...
                              nanosecond};
  if (has_time_param) {
    if (!(truncate && time_param_offset_second != tz_offset_seconds_ &&
          time_param_offset_second != 0)) {
      ns = std::chrono::nanoseconds(ns.count() -
                                    time_param_offset_second * 1000000000 +
                                    tz_offset_seconds_ * 1000000000);
    }
  }
  date::local_time<std::chrono::nanoseconds> tp{ns};
...
std::string Time::GetTimezoneName() const { return timezone_name_; }

Value Time::GetUnit(std::string unit) const {
  std::transform(unit.begin(), unit.end(), unit.begin(), ::tolower);
  if (unit == "hour") {
    return Value::Integer(nanoseconds_since_today_with_of_ / 3600000000000 %
                          24);
  } else if (unit == "minute") {
...
bool Time::operator<(const Time& rhs) const noexcept {
  return nanoseconds_since_today_with_of_ -
             tz_offset_seconds_ * NANOS_PER_SECOND <
         rhs.nanoseconds_since_today_with_of_ -
             rhs.tz_offset_seconds_ * NANOS_PER_SECOND;
}
...
Time Time::operator-(const Duration& duration) const {
  auto nanos = (nanoseconds_since_today_with_of_ -
