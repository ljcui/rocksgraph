...
namespace common {
Duration::Duration(int64_t m, int64_t d, int64_t s, int64_t n)
    : months(m), days(d), seconds(s), nanos(n) {
...
Duration Duration::approximate(double m, double d, double s, double n,
                               int sign) {
  ...
  double daysDiffInNanos =
      NANOS_PER_SECOND * SECONDS_PER_DAY * (d - daysAsLong);
  ...
  return Duration(sign * monthsAsLong, sign * daysAsLong, sign * secondsAsLong,
                  sign * nanosAsLong);
}
...
double Duration::parseFractional(const std::string &input, size_t pos) {
  return std::stod(input[pos] == '.'
                       ? input
                       : (input.substr(0, pos) + "." + input.substr(pos + 1)));
}
...
int64_t Duration::getTimeValue(const Value &d) {
  if (d.IsDate()) {
    ...
    return std::get<0>(d.AsDateTime().GetStorage()) -
           std::get<1>(d.AsDateTime().GetStorage()) * NANOS_PER_SECOND;
  } else if (d.IsLocalDateTime()) {
    return d.AsLocalDateTime().GetStorage();
  } else if (d.IsTime()) {
    return std::get<0>(d.AsTime().GetStorage()) -
           std::get<1>(d.AsTime().GetStorage()) * NANOS_PER_SECOND;
...
bool Duration::hasZone(const Value &d) { return d.IsDateTime() || d.IsTime(); }
...
Duration Duration::between(const Value &from, const Value &to,
  ...
  if (hasDate(from) && !hasDate(to)) {
    to_nanos = to_nanos % (SECONDS_PER_DAY * NANOS_PER_SECOND) +
               getTimeValueWithOffset(from) /
                   (SECONDS_PER_DAY * NANOS_PER_SECOND) *
                   (SECONDS_PER_DAY * NANOS_PER_SECOND);
  } else if (!hasDate(from) && hasDate(to)) {
    from_nanos = from_nanos % (SECONDS_PER_DAY * NANOS_PER_SECOND) +
                 getTimeValueWithOffset(to) /
                     (SECONDS_PER_DAY * NANOS_PER_SECOND) *
                     (SECONDS_PER_DAY * NANOS_PER_SECOND);
  }
  ...
  if (std::abs(bet) >= SECONDS_PER_DAY * NANOS_PER_SECOND) {
    ...
    auto start_ymd = date::year_month_day{
        date::floor<date::days>(date::local_time<std::chrono::nanoseconds>(
            std::chrono::nanoseconds(from_nanos)))};
    auto end_ymd = date::year_month_day{
        date::floor<date::days>(date::local_time<std::chrono::nanoseconds>(
            std::chrono::nanoseconds(to_nanos)))};
    month = (date::year_month(end_ymd.year(), end_ymd.month()) -
             date::year_month(start_ymd.year(), start_ymd.month()))
                .count();
    all_day = (date::sys_days(end_ymd) - date::sys_days(start_ymd)).count();
    if (month > 0 && (start_ymd + date::months(month) > end_ymd ||
                      (start_ymd + date::months(month) == end_ymd &&
                       from_day_nanos > to_day_nanos))) {
      month--;
    }
    if (month < 0 && (start_ymd + date::months(month) < end_ymd ||
                      (start_ymd + date::months(month) == end_ymd &&
...
Duration Duration::parseDuration(int sign, int64_t month, int64_t day,
                                 std::smatch &match, bool strict,
                                 const std::string &hour,
                                 const std::string &min, const std::string &sec,
                                 const std::string &sub) {
  if (!strict) {
    size_t pos;
    if ((pos = fractionPoint(hour)) != std::string::npos) {
      if (!min.empty() || !sec.empty()) {
        THROW_CODE(InvalidParameter, "Text cannot be parsed to a Duration {}",
                   match[0].str());
      }
      return approximate(month, day, parseFractional(hour, pos) * 3600, 0,
                         sign);
    }
    ...
    if ((pos = fractionPoint(min)) != std::string::npos) {
      ...
      return approximate(month, day,
                         secondsFromHours + parseFractional(min, pos) * 60, 0,
                         sign);
    }
  }
  ...
  if (match[DURATION_TIME].matched) {
    if (match[DURATION_LONG_HOUR].matched) {
      return parseDuration(
          sign, months, days, match, true, match[DURATION_LONG_HOUR],
          match[DURATION_LONG_MINUTE], match[DURATION_LONG_SECOND],
          match[DURATION_LONG_SUB]);
    } else {
      return parseDuration(
          sign, months, days, match, true, match[DURATION_SHORT_HOUR],
          match[DURATION_SHORT_MINUTE], match[DURATION_SHORT_SECOND],
          match[DURATION_SHORT_SUB]);
    }
  } else {
...
Duration Duration::parseDuration(std::smatch &match) {
  ...
  if ((y.empty() && m.empty() && w.empty() && d.empty() && t.empty()) ||
      "T" == t || "t" == t) {
    THROW_CODE(InvalidParameter, "Text cannot be parsed to a Duration {}",
               match[0].str());
  }
  ...
  daysAcc += days;
  return parseDuration(
      sign, monthsAcc, daysAcc, match, false, match[DURATION_HOURS].str(),
      match[DURATION_MINUTES].str(), match[DURATION_SECONDS].str(),
      match[DURATION_SUB_SECONDS].str());
}
...
Duration::Duration(const Value &params) {
  ...
  if (allIntegralValues) {
    *this = Duration(parse_params_map["years"].AsInteger() * 12 +
                         parse_params_map["months"].AsInteger(),
                     parse_params_map["weeks"].AsInteger() * 7 +
                         parse_params_map["days"].AsInteger(),
                     parse_params_map["hours"].AsInteger() * 3600 +
                         parse_params_map["minutes"].AsInteger() * 60 +
                         parse_params_map["seconds"].AsInteger(),
                     parse_params_map["milliseconds"].AsInteger() * 1000000 +
                         parse_params_map["microseconds"].AsInteger() * 1000 +
                         parse_params_map["nanoseconds"].AsInteger());
  } else {
    *this = approximate(
        safeCastFloatingPoint(parse_params_map["years"]) * 12 +
            safeCastFloatingPoint(parse_params_map["months"]),
        safeCastFloatingPoint(parse_params_map["weeks"]) * 7 +
            safeCastFloatingPoint(parse_params_map["days"]),
        safeCastFloatingPoint(parse_params_map["hours"]) * 3600 +
            safeCastFloatingPoint(parse_params_map["minutes"]) * 60 +
            safeCastFloatingPoint(parse_params_map["seconds"]),
        safeCastFloatingPoint(parse_params_map["milliseconds"]) * 1000000 +
            safeCastFloatingPoint(parse_params_map["microseconds"]) * 1000 +
            safeCastFloatingPoint(parse_params_map["nanoseconds"]));
  }
}
...
std::string Duration::ToString() const {
  if (months == 0 && days == 0 && seconds == 0 && nanos == 0) {
    return "PT0S";  // no need to allocate a string builder if we know the
                    // result
  }
  ...
  if (str.length() == 1) {  // this was all zeros (but not ZERO for some
                            // reason), ensure well formed output:
    str.append("T0S");
  }
  return str;
}
...
bool Duration::operator!=(const Duration &rhs) const noexcept {
  return months != rhs.months || days != rhs.days || seconds != rhs.seconds ||
         nanos != rhs.nanos;
}

Duration Duration::operator+(const Duration &duration) const {
  return Duration(months + duration.months, days + duration.days,
                  seconds + duration.seconds, nanos + duration.nanos);
}
...
Duration Duration::operator*(double num) const {
  return Duration::approximate(months * num, days * num, seconds * num,
                               nanos * num);
}
