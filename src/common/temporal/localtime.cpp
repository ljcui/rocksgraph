...
void LocalTime::fromTimeZone(std::string timezone) {
  ...
  try {
    ...
    auto current_time =
        date::make_zoned(current_time_zone, std::chrono::system_clock::now());
    nanoseconds_since_today_ =
        current_time.get_local_time().time_since_epoch().count() +
...
LocalTime LocalTime::operator-(const Duration& duration) const {
  auto nanos = (nanoseconds_since_today_ - duration.seconds * NANOS_PER_SECOND -
                duration.nanos) %
               (NANOS_PER_SECOND * SECONDS_PER_DAY);
  if (nanos < 0) {
