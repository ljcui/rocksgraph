...
void validateDateParams(
  ...
  const static std::unordered_map<std::string, std::string> fieldGroup{
      {DATE_DAY, DATE_MONTH}, {DATE_DOQ, DATE_QUARTER}, {DATE_DOW, DATE_WEEK}};
  ...
  // handle field group
  if (params_map.count(DATE_DATE) != 0 &&
      params_map.count(DATE_DATETIME) != 0) {
    for (const auto& [param, _] : params_map) {
      if (fieldGroup.count(param)) {
        if (params_map.count(fieldGroup.at(param)) == 0) {
          THROW_CODE(InputError, "{} can not be specified without {}", param,
                     fieldGroup.at(param));
        }
      }
    }
...
void Date::fromYearMonthDay(const Date& base_date, std::optional<int> year,
  ...
  date::year_month_day ymd = date::year_month_day{
      date::local_days(date::days(base_date.days_since_epoch_))};
  int target_year = year.has_value() ? year.value() : (int)ymd.year();
  unsigned target_month =
      month.has_value() ? month.value() : (unsigned)ymd.month();
  unsigned target_day = day.has_value() ? day.value() : (unsigned)ymd.day();

  ymd = date::year_month_day(date::year{target_year}, date::month{target_month},
...
void Date::fromYearWeekDow(const Date& base_date, std::optional<int> year,
  ...
  unsigned target_dow = dow.has_value() ? dow.value() : (unsigned)yww.weekday();
...
void Date::fromYearQuarterDoq(const Date& base_date, std::optional<int> year,
                              std::optional<int> quarter,
                              std::optional<int> doq) {
  date::year_month_day ymd = date::year_month_day{
      date::local_days(date::days(base_date.days_since_epoch_))};
  int target_year = year.has_value() ? year.value() : (int)ymd.year();
  if (!doq.has_value()) {
    unsigned target_month =
        quarter.has_value()
            ? (quarter.value() - 1) * 3 + ((unsigned)ymd.month() - 1) % 3 + 1
            : (unsigned)ymd.month();
    ymd = date::year_month_day(date::year(target_year),
                               date::month(target_month), ymd.day());
  } else {
    unsigned target_quarter = quarter.has_value()
                                  ? quarter.value()
                                  : ((unsigned)ymd.month() - 1) / 3 + 1;
...
void Date::fromYearDoy(const Date& base_date, std::optional<int> year,
                       std::optional<int> doy) {
  date::year_month_day ymd = date::year_month_day{
      date::local_days(date::days(base_date.days_since_epoch_))};
  int target_year = year.has_value() ? year.value() : (int)ymd.year();
  if (doy.has_value()) {
    ymd = date::year_month_day{date::year{target_year}, date::month{1},
                               date::day{1}};
    auto days = (date::local_days)ymd;
...
void Date::fromTimeZone(std::string timezone) {
  ...
  try {
    if (timezone[0] == '+' || timezone[0] == '-') {
      ...
      if (timezone[0] == '-') {
        current_time =
            date::make_zoned(date::current_zone(), now - offset.to_duration());
      } else {
        current_time =
            date::make_zoned(date::current_zone(), now + offset.to_duration());
      }
...
Date::Date(const Value& params) {
  ...
  if (params.IsDateTime()) {
    days_since_epoch_ = std::get<0>(params.AsDateTime().GetStorage()) /
                        (NANOS_PER_SECOND * SECONDS_PER_DAY);
    return;
  }
  if (params.IsLocalDateTime()) {
    days_since_epoch_ = params.AsLocalDateTime().GetStorage() /
                        (NANOS_PER_SECOND * SECONDS_PER_DAY);
    return;
  }
  ...
  try {
    if (params_map.count(DATE_DATE)) {
      if (params_map.at(DATE_DATE).IsDate()) {
        ...
        base_date = Date(
            std::get<0>(params_map.at(DATE_DATE).AsDateTime().GetStorage()) /
            (NANOS_PER_SECOND * SECONDS_PER_DAY));
      } else if (params_map.at(DATE_DATE).IsLocalDateTime()) {
        base_date =
            Date(params_map.at(DATE_DATE).AsLocalDateTime().GetStorage() /
                 (NANOS_PER_SECOND * SECONDS_PER_DAY));
      } else {
        THROW_CODE(InvalidParameter, "Cannot get the date of: {}",
                   params_map.at(DATE_DATE).ToString());
      }
    }
  ...
  switch (dt) {
    ...
    case DateType::TIMEZONE_DATE:
      ...
      break;
    default:
      THROW_CODE(InputError, "Failed to parse {} into Date", params.ToString());
  }
}
...
Value Date::GetUnit(std::string unit) const {
  ...
  iso_week::year_weeknum_weekday yww(
      date::local_days{date::days{days_since_epoch_}});
  if (unit == "year") {
    ...
    return Value::Integer(
        ...
        date::local_days(
            date::year_month_day{
                ymd.year(),
                date::month(((unsigned(ymd.month()) - 1) / 3) * 3 + 1),
                date::day(1)})
            .time_since_epoch()
            .count() +
        1);
