...
std::string Value::Serialize() const {
  ...
  switch (type) {
    ...
    default: {
      THROW_CODE(ValueException,
                 "Unsupported data type for value serializing, type: " +
                     ::ToString(type));
    }
  }
  return buffer;
...
void Value::Deserialize(const char* p, size_t size) {
  ...
  switch (type) {
      ...
      while (size > 0) {
        switch (t) {
          ...
          default: {
            THROW_CODE(
                ValueException,
                "unexpected data type for Deserialize : " + ::ToString(t));
          }
        }
      }
      break;
    ...
    default: {
      THROW_CODE(ValueException,
                 "unexpected data type for Deserialize : " + ::ToString(type));
    }
  }
}
...
std::string ToString(ValueType t) {
  switch (t) {
    ...
    case ValueType::DURATION:
      return "DURATION";
    default:
      THROW_CODE(ValueException, "unexpected ValueType for Type ToString");
  }
}
...
std::string Value::ToString(bool str_quotation_mark) const {
  switch (type) {
    ...
    case ValueType::MAP: {
      const auto& d =
          std::any_cast<const std::unordered_map<std::string, Value>&>(data);
      return fmt::format("{}", d);
    }
    case ValueType::DATE: {
    ...
    default:
      THROW_CODE(ValueException, "unexpected ValueType for Value ToString");
  }
}
  ...
  switch (type) {
    ...
    case ValueType::DATETIME: {
      ...
      if (std::get<0>(s.GetStorage()) % 1000000000 >= 0) {
        ...
        return bolt::DateTime{
            std::get<0>(s.GetStorage()) / 1000000000 - 1,
    ...
    case ValueType::LOCALDATETIME: {
      ...
      if (s % 1000000000 >= 0) {
        ...
        return bolt::LocalDateTime{s / 1000000000 - 1,
                                   s % 1000000000 + 1000000000};
      }
    }
    case ValueType::LOCALTIME: {
      return bolt::LocalTime{
          std::any_cast<const common::LocalTime&>(data).GetStorage()};
    }
    case ValueType::TIME: {
    ...
    case ValueType::DURATION: {
      return bolt::Duration{
          std::any_cast<const common::Duration&>(data).months,
    ...
    switch (type) {
      ...
      case ValueType::STRING:
        return std::any_cast<const std::string&>(data) >
               std::any_cast<const std::string&>(rhs.data);
      case ValueType::DATE:
        return std::any_cast<const common::Date>(data) >
               std::any_cast<const common::Date&>(rhs.data);
      case ValueType::DATETIME:
      ...
      case ValueType::LOCALDATETIME:
        return std::any_cast<const common::LocalDateTime>(data) >
               std::any_cast<const common::LocalDateTime&>(rhs.data);
      case ValueType::LOCALTIME:
        return std::any_cast<const common::LocalTime>(data) >
               std::any_cast<const common::LocalTime&>(rhs.data);
      case ValueType::TIME:
        return std::any_cast<const common::Time>(data) >
               std::any_cast<const common::Time&>(rhs.data);
      case ValueType::DURATION:
        return std::any_cast<const common::Duration>(data) >
               std::any_cast<const common::Duration&>(rhs.data);
      default:
        THROW_CODE(ValueException, "Unhandled data type");
    }
  } catch (std::exception& e) {
    THROW_CODE(ValueException,
               "Unable to compare two Value with different types. " +
                   ::ToString(type) + " vs " + ::ToString(rhs.type));
  }
}
  ...
  switch (this->type) {
    ...
    case ValueType::INTEGER: {
      return std::any_cast<int64_t>(this->data) ==
             std::any_cast<int64_t>(b.data);
    }
    case ValueType::DOUBLE: {
      return std::abs(std::any_cast<double>(this->data) -
                      std::any_cast<double>(b.data)) <
             std::numeric_limits<double>::epsilon();
    }
    case ValueType::FLOAT: {
      return std::abs(std::any_cast<float>(this->data) -
                      std::any_cast<float>(b.data)) <
             std::numeric_limits<float>::epsilon();
    }
    case ValueType::STRING: {
      return std::any_cast<const std::string&>(this->data) ==
             std::any_cast<const std::string&>(b.data);
    }
    case ValueType::ARRAY: {
    ...
    case ValueType::MAP: {
      const auto& l =
          std::any_cast<const std::unordered_map<std::string, Value>&>(
              this->data);
      const auto& r =
          std::any_cast<const std::unordered_map<std::string, Value>&>(b.data);
      if (l.size() != r.size()) {
        return false;
      }
    ...
    case ValueType::DATE:
      return std::any_cast<const common::Date>(this->data) ==
             std::any_cast<const common::Date&>(b.data);
    case ValueType::LOCALDATETIME:
      return std::any_cast<const common::LocalDateTime>(this->data) ==
             std::any_cast<const common::LocalDateTime&>(b.data);
    case ValueType::DATETIME:
      return std::any_cast<const common::DateTime>(this->data) ==
             std::any_cast<const common::DateTime&>(b.data);
    case ValueType::LOCALTIME:
      return std::any_cast<const common::LocalTime>(data) ==
             std::any_cast<const common::LocalTime&>(b.data);
    case ValueType::TIME:
      return std::any_cast<const common::Time>(data) ==
             std::any_cast<const common::Time&>(b.data);
    case ValueType::DURATION:
      return std::any_cast<const common::Duration>(data) ==
             std::any_cast<const common::Duration&>(b.data);
    case ValueType::Null: {
      return true;
    }
  }
