...
std::optional<bool> ExtractBoolPointer(
    std::unordered_map<std::string, std::any> counters,
...
void Hydrator::ClearErr() { err_.reset(); }

void Hydrator::UseUtc(bool use) { useUtc_ = use; }
...
void Hydrator::AssertLength(const std::string& structType, uint32_t expected,
                            uint32_t actual) {
  if (expected != actual) {
    SetErr(fmt::format("Invalid length of struct, expected {} but was {}",
                       expected, actual));
  }
}

// Hydrate hydrates a top-level struct message
std::pair<std::any, std::optional<std::string>> Hydrator::Hydrate(
    std::string_view buf) {
  unp_ = &unpacker_;
  unp_->Reset(buf);
  unp_->Next();
...
std::any Hydrator::GetDateTimeOffset(uint32_t n) {
  ...
  return LegacyDateTime{
      .seconds = seconds, .nanoseconds = nanos, .tz_offset_seconds = offset};
}
...
std::any Hydrator::GetDateTimeNamedZone(uint32_t n) {
  ...
  return LegacyDateTimeZoneId{
      .seconds = seconds, .nanoseconds = nanos, .tz_id = zone};
}
...
std::vector<Notification> ParseNotifications(
    std::vector<std::any> notificationsx) {
...
ParsePlanOpIdArgsChildren(std::unordered_map<std::string, std::any> planx) {
  ...
  std::vector<std::any> identifiersx;
  {
    ...
    if (iter != planx.end()) {
      identifiersx =
          std::any_cast<std::vector<std::any>&&>(std::move(iter->second));
    }
  }
  ...
  return {std::move(oper), std::move(identifiers), std::move(arguments),
          std::move(childrenx)};
}
