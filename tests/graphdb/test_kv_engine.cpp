...
TEST(TxnKV, basic1) {
  ...
  auto iter = txn->GetIterator(ro);
  for (iter->Seek("key0"); iter->Valid(); iter->Next()) {
    LOG_INFO("key1: {}, val:{}", iter->key().ToString(),
             iter->value().ToString());
    std::string new_val("new_");
    new_val.append(iter->key().ToString());
    txn->Put(iter->key(), new_val);
    LOG_INFO("key2: {}, val:{}", iter->key().ToString(),
             iter->value().ToString());
  }
  for (iter->Seek("key0"); iter->Valid(); iter->Next()) {
    LOG_INFO("key3: {}, val:{}", iter->key().ToString(),
             iter->value().ToString());
  }
  for (iter->Seek("key0"); iter->Valid(); iter->Next()) {
    LOG_INFO("key4: {}, val:{}", iter->key().ToString(),
             iter->value().ToString());
    txn->Delete(iter->key());
  }
  for (iter->Seek("key0"); iter->Valid(); iter->Next()) {
    LOG_INFO("key5: {}, val:{}", iter->key().ToString(),
             iter->value().ToString());
  }
  delete iter;
  txn->Commit();
...
TEST(TxnKV, basic2) {
  ...
  for (; iter->Valid(); iter->Next()) {
    LOG_INFO("key: {}, val:{}", iter->key().ToString(),
             iter->value().ToString());
  }
  delete iter;
  txn->Commit();
