...
DEFINE_int32(num, 10000,
             "The number of inserted vertexes, the number of edges is ten "
             "times the number of points");
DEFINE_int32(
    thread, 16,
    "The number of threads that perform benchmark read and write operations");
...
class BenchmarkLightningGraph {
 ...
 public:
  BenchmarkLightningGraph(GraphDB& db, size_t vertices)
      : db(db), vertices(vertices) {}
...
#pragma omp parallel
    {
      ...
      auto start = local_count * thread_id,
           end = std::min(local_count * (thread_id + 1), count);
...
TEST(Benchmark, DISABLED_khop) {
  ...
  if (thread > 1) {
    LOG_INFO("The throughput of {} vertexes insertion is {} op/s ({} threads)",
             n, bm.test_write_vertex_mt(n, thread), thread);
    LOG_INFO("The throughput of {} edges insertion is {} op/s ({} threads)",
             n * 10, bm.test_write_edge_mt(n * 10, thread), thread);
    for (int i = 1; i <= depth; ++i) {
      auto query = std::max(int(n / std::pow(10, i - 1)), 100);
      LOG_INFO("The throughput of {} vertexes {}-hop is {} op/s ({} threads)",
               query, i, bm.test_read_neighbour_mt(query, i, thread), thread);
    }
  } else {
    LOG_INFO("The time of {} vertexes insertion is {}s", n,
             bm.test_write_vertex(n));
    LOG_INFO("The time of {} edges insertion is {}s", n * 10,
             bm.test_write_edge(n * 10));
    for (int i = 1; i <= depth; ++i) {
      auto query = std::max(int(n / std::pow(10, i - 1)), 100);
      LOG_INFO("The time of {} vertexes {}-hop is {}s", query, i,
               bm.test_read_neighbour(query, i));
    }
  }
}
