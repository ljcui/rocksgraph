...
TEST(PhysicalPlanTest, AllocatesSlotsForOptionalExpand) {
  PlannedQuery query =
      Plan("MATCH (n) OPTIONAL MATCH (n)-[r]->(m) RETURN n, r, m");
  rg::PhysicalPlan physical = rg::CreatePhysicalPlan(query.LogicalPlan());

  EXPECT_TRUE(physical.Root().output_slots->Contains("n"));
  EXPECT_TRUE(physical.Root().output_slots->Contains("r"));
  EXPECT_TRUE(physical.Root().output_slots->Contains("m"));
  ...
  EXPECT_TRUE(expand_node->children[0]->output_slots->Contains("n"));
}

TEST(PhysicalPlanTest, BuildsUnionSlotLayout) {
  PlannedQuery query =
      Plan("MATCH (n) RETURN n AS value UNION ALL RETURN 1 AS value");
  rg::PhysicalPlan physical = rg::CreatePhysicalPlan(query.LogicalPlan());

  EXPECT_TRUE(physical.Root().output_slots->Contains("value"));
...
TEST(PhysicalPlanTest, UnionOperatorsHandleResourcesAndEarlyClose) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(all, graph, {}, {"value"});
  std::vector<rg::Value> row;
...
TEST(PhysicalPlanTest, BuildsOwnedExistenceApplyPayloads) {
  ...
  EXPECT_TRUE(std::holds_alternative<rg::LetSemiApplyOp>(let_node->data));
...
TEST(PhysicalPlanTest, ExecutesDetachedExistenceApplyOperators) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(let, graph, {}, {"id", "has"});
  std::vector<rg::Value> row;
  ASSERT_TRUE(cursor->Next(&row));
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
...
TEST(PhysicalPlanTest, BuildsOwnedRollUpApplyPayload) {
  ...
  EXPECT_TRUE(std::holds_alternative<rg::RollUpApplyOp>(roll_up->data));
...
TEST(PhysicalPlanTest, ExecutesDetachedRollUpApplyAndHandlesResources) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(physical, graph, {}, {"id", "names"});
  std::vector<rg::Value> row;
  ASSERT_TRUE(cursor->Next(&row));
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
...
TEST(PhysicalPlanTest, RemainingUnaryOperatorsHandleResources) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(procedure, graph, {}, {"label"});
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
...
TEST(PhysicalPlanTest, DeleteOperatorHandlesCloseCancellationAndMemoryLimit) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(physical, graph, {}, {"n"});
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
  EXPECT_EQ(graph.Nodes().size(), 1U);
...
TEST(PhysicalPlanTest, ReinstantiatesStatefulApplyRightSideForEachLeftRow) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(physical, graph, {}, {"n", "m"});
  std::vector<rg::Value> row;
  ASSERT_TRUE(cursor->Next(&row));
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
...
TEST(PhysicalPlanTest, BuildsSlotsForExpressions) {
  PlannedQuery query = Plan("MATCH (n) RETURN [n][0] AS x");
  rg::PhysicalPlan physical = rg::CreatePhysicalPlan(query.LogicalPlan());

  EXPECT_TRUE(physical.Root().output_slots->Contains("x"));
}
...
TEST(PhysicalPlanTest, BuildsOwnedPayloadsForMigratedOperators) {
  ...
  const rg::PhysicalPlanNode *projection_node =
      FindPhysicalPlan(physical.Root(), rg::PhysicalOperatorKind::kProjection);
  ...
  EXPECT_NE(FindPhysicalPlan(physical.Root(), rg::PhysicalOperatorKind::kSkip),
            nullptr);
  EXPECT_NE(FindPhysicalPlan(physical.Root(), rg::PhysicalOperatorKind::kLimit),
            nullptr);
...
TEST(PhysicalPlanTest, ExecutesMigratedPlanAfterLogicalPlanIsDestroyed) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(physical, graph, {},
...
TEST(PhysicalPlanTest, BuildsTypedOwnedPayloadsForFixedTraversalOperators) {
  {
    ...
    ASSERT_NE(node, nullptr);
    const auto &data = std::get<rg::ExpandOp>(node->data);
  ...
  }
  {
    ...
    ASSERT_NE(node, nullptr);
    const auto &data = std::get<rg::ExpandIntoOp>(node->data);
    EXPECT_EQ(data.pattern.from_node, "a");
    EXPECT_EQ(data.pattern.to_node, "b");
    EXPECT_EQ(data.pattern.direction, rg::PhysicalExpandDirection::kOutgoing);
    ExpectSameOffset(data.from_node_input_slot,
  ...
  }
  {
    ...
    const rg::PhysicalPlanNode *node = FindPhysicalPlan(
        physical.Root(), rg::PhysicalOperatorKind::kOptionalExpand);
    ASSERT_NE(node, nullptr);
    const auto &data = std::get<rg::OptionalExpandOp>(node->data);
    ASSERT_EQ(data.predicates.size(), logical->Predicates().size());
    EXPECT_NE(data.predicates.front().Expression(),
              logical->Predicates().front());
    ExpectSameOffset(data.from_node_input_slot,
...
TEST(PhysicalPlanTest, BuildsTypedOwnedPayloadsForVariableTraversalOperators) {
  {
    ...
    ASSERT_NE(node, nullptr);
    const auto &data = std::get<rg::VarExpandOp>(node->data);
    EXPECT_EQ(data.pattern.from_node, "a");
    EXPECT_EQ(data.pattern.relationship, "rs");
    EXPECT_EQ(data.pattern.to_node, "b");
    EXPECT_EQ(data.pattern.direction, rg::PhysicalExpandDirection::kOutgoing);
  ...
  }
  {
    ...
    ASSERT_NE(node, nullptr);
    const auto &data = std::get<rg::PathBuildOp>(node->data);
...
TEST(PhysicalPlanTest, ClosesFixedExpandCursorEarly) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(physical, graph, {}, {"relationship_id"});
  std::vector<rg::Value> row;
  ASSERT_TRUE(cursor->Next(&row));
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
...
TEST(PhysicalPlanTest, ClosesVariableExpandCursorEarlyAndEnforcesMemoryLimit) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(physical, graph, {}, {"b"});
  std::vector<rg::Value> row;
  ASSERT_TRUE(cursor->Next(&row));
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
...
TEST(PhysicalPlanTest, StreamsCartesianProductAfterLogicalPlanIsDestroyed) {
  ...
  std::unique_ptr<rg::PhysicalResultCursor> cursor =
      rg::StartGraphDBPhysicalPlan(physical, graph, {}, {"a", "b"});
  std::vector<rg::Value> row;
  ASSERT_TRUE(cursor->Next(&row));
  cursor->Close();
  EXPECT_FALSE(cursor->Next(&row));
...
TEST(PhysicalPlanTest, PrintsOwnedOrderingAfterLogicalPlanAndAstAreDestroyed) {
  ...
  EXPECT_NE(printed.find("slots=[a@0"), std::string::npos);
}
