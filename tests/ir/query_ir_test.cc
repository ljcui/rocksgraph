...
void ExpectQueryIRText(const std::string &query, const std::string &expected) {
  auto statement = ParseOrFail(query);
  ASSERT_TRUE(statement);
  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  EXPECT_EQ(ir::QueryIRToString(*query_ir), expected);
}

void ExpectQueryIRContractError(const std::string &query,
...
TEST(QueryIRInputContractTest, RejectsRawReturnStar) {
  ExpectQueryIRContractError("MATCH (n) RETURN *",
                             "projection star must be expanded");
}

TEST(QueryIRInputContractTest, RejectsRawProjectionWithoutAlias) {
  ExpectQueryIRContractError("MATCH (n) RETURN n",
                             "projection item alias must be filled");
}

TEST(QueryIRInputContractTest, RejectsRawInlineReadPatternPredicates) {
  ExpectQueryIRContractError(
      "MATCH (n:Person {id: 1}) RETURN n AS n",
      "inline node labels in read patterns must be normalized");
}

TEST(QueryIRInputContractTest, RejectsRawPatternPredicateExpression) {
  ExpectQueryIRContractError(
      "MATCH (n) WHERE (n)-->() RETURN n AS n",
      "pattern predicates must be rewritten to existential subqueries");
}

TEST(QueryIRInputContractTest, RejectsRawAnonymousPatternNames) {
  ExpectQueryIRContractError("MATCH () RETURN 1 AS x",
                             "anonymous nodes must be named");
}

TEST(QueryIRInputContractTest, AcceptsRewrittenReturnStar) {
  auto statement = ParseOrFail("MATCH (n) RETURN *");
  ASSERT_TRUE(statement);

  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::RegularQueryProjection &projection =
      query_ir->RequireSingle().horizon.RequireRegularProjection();
...
TEST(QueryIRPrinterTest, DumpsInlineNodePredicateSelection) {
  ExpectQueryIRText("MATCH (n:Person) RETURN n", R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
    pattern_relationships:
...
TEST(QueryIRPrinterTest, DumpsRelationshipPatternWithInlineType) {
  ExpectQueryIRText("MATCH (a)-[r:KNOWS]->(b) RETURN a, b",
                    R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [a, b]
...
TEST(QueryIRPrinterTest, DumpsExplicitRelationshipTypeSelection) {
  ExpectQueryIRText("MATCH (a)-[r]->(b) WHERE r:KNOWS RETURN r",
                    R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [a, b]
...
TEST(QueryIRPrinterTest, DumpsVariableLengthRelationship) {
  ExpectQueryIRText("MATCH (a)-[r*1..3]->(b) RETURN r",
                    R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [a, b]
...
TEST(QueryIRPrinterTest, DumpsUnwindTail) {
  ExpectQueryIRText("UNWIND [1, 2] AS x RETURN x", R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: []
    pattern_relationships:
...
TEST(QueryIRPrinterTest, DumpsUnionAll) {
  ExpectQueryIRText("RETURN 1 AS x UNION ALL RETURN 2 AS x",
                    R"(UnionQueryIR
  all: true
  distinct: false
  mappings:
...
TEST(QueryIRPrinterTest, DumpsInlinePropertySelection) {
  ExpectQueryIRText("MATCH (n {id: 1}) RETURN n", R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
    pattern_relationships:
...
TEST(QueryIRPrinterTest, DumpsExplicitWhereSelection) {
  ExpectQueryIRText("MATCH (n) WHERE n.age > 30 RETURN n",
                    R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
...
TEST(QueryIRPrinterTest, DumpsStructuredPropertyPredicates) {
  ExpectQueryIRText(
...
TEST(QueryIRPrinterTest, DumpsWithTail) {
  ExpectQueryIRText("MATCH (n) WITH n.name AS name RETURN name",
                    R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
...
TEST(QueryIRPrinterTest, DumpsDistinctProjection) {
  ExpectQueryIRText("MATCH (n) RETURN DISTINCT n", R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
    pattern_relationships:
...
TEST(QueryIRPrinterTest, DumpsCountStarSnapshot) {
  ExpectQueryIRText("MATCH (n) RETURN count(*)", R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
    pattern_relationships:
...
TEST(QueryIRPrinterTest, DumpsOrderBySkipLimit) {
  ExpectQueryIRText("MATCH (n) RETURN n ORDER BY n SKIP 1 LIMIT 2",
                    R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
...
TEST(QueryIRPrinterTest, DumpsRequiredOrderDirections) {
  ExpectQueryIRText("MATCH (n) RETURN n ORDER BY n.name DESC, n.age ASC",
                    R"(SingleQueryIR
  query_graph:
    argument_ids: []
    pattern_nodes: [n]
...
TEST(QueryIRTest, BuildsGraphFromMatch) {
  ...
  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::SingleQueryIR &main = query_ir->RequireSingle();
  ...
  EXPECT_EQ(query_ir->Kind(), ir::QueryIRKind::kSingle);
}

TEST(QueryIRTest, BuildsNamedPathBinding) {
...
TEST(QueryIRTest, NarrowsNamedPathTailArgumentIds) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, BuildsRegularProjectionHorizon) {
  auto statement = ParseOrFail("MATCH (n) RETURN n");
  ASSERT_TRUE(statement);

  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::QueryHorizon &horizon = query_ir->RequireSingle().horizon;
...
TEST(QueryIRTest, BuildsDistinctProjectionHorizon) {
...
TEST(QueryIRTest, BuildsAggregatingProjectionItems) {
  auto statement =
...
TEST(QueryIRTest, PreservesMixedAggregationProjectionOrder) {
  auto statement =
...
TEST(QueryIRTest, QueriesSelectionsByStructuredPredicateFields) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, ClassifiesNormalizedPropertyPredicates) {
  auto statement = ParseOrFail(
  ...
  const ir::Selections &selections =
      query_ir->RequireSingle().query_graph.selections;
...
TEST(QueryIRTest, SelectionsExposePushdownAndInequalityGroups) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, SelectionsExtractNestedPredicateInfo) {
  auto statement =
...
TEST(QueryIRTest, QueryGraphApisExposeCoveredIdsAndAvailability) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, QueryGraphApisSummarizeLabelsTypesAndProperties) {
  auto statement = ParseOrFail(
      "MATCH (a:Person {name: 'Alice'})-[r:KNOWS]->(b) "
...
TEST(QueryIRTest, QueryGraphApisFindConnectedComponents) {
  auto statement =
...
TEST(QueryIRTest, AcceptsAnonymousPatternAfterRewrite) {
...
TEST(QueryIRTest, BuildsVariableLengthRelationshipPattern) {
...
TEST(QueryIRTest, BuildsProjectionSelectionsForWithWhere) {
  auto statement =
...
TEST(QueryIRTest, KeepsDistinctWithAsEventHorizon) {
  auto statement =
...
TEST(QueryIRTest, NarrowsProjectionExistsSubqueryArgumentIds) {
  auto statement = ParseOrFail(
  ...
  const ir::SingleQueryIR &subquery = predicate.subquery->RequireSingle();
...
TEST(QueryIRTest, ClassifiesNotExistsSubqueryPredicate) {
  auto statement = ParseOrFail(
  ...
  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::SingleQueryIR &query = query_ir->RequireSingle();
...
TEST(QueryIRTest, InlinedWithExistsDoesNotCaptureFutureVariables) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, BuildsProjectionPagination) {
...
TEST(QueryIRTest, BuildsProjectionRequiredOrder) {
  auto statement =
...
TEST(QueryIRTest, BuildsProjectionPositionAndInterestingOrder) {
...
TEST(QueryIRTest, MarksWithProjectionAsIntermediate) {
...
TEST(QueryIRTest, NarrowsTailArgumentIdsToActualDependencies) {
  auto statement =
...
TEST(QueryIRTest, BuildsOptionalMatchQueryGraph) {
  auto statement = ParseOrFail(
      "MATCH (a) WITH a, 1 AS unused "
  ...
  const ir::SingleQueryIR &first = query_ir->RequireSingle();
  ASSERT_TRUE(first.tail);
  const ir::SingleQueryIR &main = *first.tail;
...
TEST(QueryIRTest, AccumulatesSequentialOptionalMatchArguments) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, AddsAssertIsNodeForStandaloneArgumentPatternNode) {
  auto statement =
...
TEST(QueryIRTest, ClassifiesArgumentRelationshipTypeWithSemanticTable) {
  auto statement =
...
TEST(QueryIRTest, ClassifiesProjectionWhereWithScopedVariableType) {
  auto statement =
...
TEST(QueryIRTest, BuildsUnwindHorizonSegment) {
  auto statement = ParseOrFail("UNWIND [1, 2] AS x RETURN x");
  ASSERT_TRUE(statement);

  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::SingleQueryIR &unwind_segment = query_ir->RequireSingle();
...
TEST(QueryIRTest, BuildsStandaloneProcedureCallHorizon) {
...
TEST(QueryIRTest, BuildsInQueryProcedureCallWithYieldWhere) {
  auto statement = ParseOrFail(
  ...
  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::SingleQueryIR &call_segment = query_ir->RequireSingle();
  ...
  const ir::SingleQueryIR &return_segment = *call_segment.tail;
...
TEST(QueryIRTest, BuildsProcedureMetadataCallAsReadOnly) {
  auto statement = ParseOrFail("CALL dbms.procedures()");
  ASSERT_TRUE(statement);

  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::ProcedureCallHorizon &call =
      query_ir->RequireSingle().horizon.RequireProcedureCall();
...
TEST(QueryIRTest, SkipsPassthroughWithAfterUnwindSegment) {
...
TEST(QueryIRTest, SplitsConjunctiveWhereIntoPredicates) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, AcceptsNonConjunctiveWhereExpression) {
...
TEST(QueryIRTest, ExcludesScopedQuantifierVariablesFromDependencies) {
  auto statement =
...
TEST(QueryIRTest, TreatsOnlyOuterPatternVariablesAsExistsDependencies) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, NarrowsExistsSubqueryArgumentIdsToUsedOuterVariables) {
  auto statement = ParseOrFail(
      "MATCH (a) WITH a, 1 AS unused "
...
TEST(QueryIRTest, BuildsNestedIRExpressionForPatternExists) {
  auto statement = ParseOrFail(
  ...
  const ir::SingleQueryIR &subquery = nested.query->RequireSingle();
  EXPECT_TRUE(Contains(subquery.query_graph.argument_ids, "n"));
  EXPECT_TRUE(Contains(subquery.query_graph.pattern_nodes, "n"));
  EXPECT_TRUE(Contains(subquery.query_graph.pattern_nodes, "m"));
  ASSERT_EQ(subquery.query_graph.pattern_relationships.size(), 1U);
...
TEST(QueryIRTest, BuildsNestedIRExpressionForPatternComprehension) {
  auto statement =
...
TEST(QueryIRTest, DeduplicatesRepeatedWherePredicatesAcrossMatches) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, DeduplicatesRepeatedNodeLabelWherePredicates) {
  auto statement =
...
TEST(QueryIRTest, DeduplicatesRepeatedRelationshipTypeWherePredicates) {
  auto statement =
...
TEST(QueryIRTest, DeduplicatesRepeatedPropertyComparisonWherePredicates) {
  auto statement =
...
TEST(QueryIRTest, DeduplicatesNormalizedPropertyPredicates) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, DeduplicatesRepeatedGenericWherePredicates) {
...
TEST(QueryIRTest, BuildsUnionBranch) {
  auto statement =
      ParseOrFail("MATCH (n) RETURN n AS x UNION MATCH (m) RETURN m AS x");
  ASSERT_TRUE(statement);

  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  ASSERT_EQ(query_ir->Kind(), ir::QueryIRKind::kUnion);

  const ir::UnionQueryIR &union_query = query_ir->RequireUnion();
  ASSERT_NE(union_query.lhs, nullptr);
  EXPECT_EQ(union_query.lhs->Kind(), ir::QueryIRKind::kSingle);
...
TEST(QueryIRTest, BuildsUnionAllBranch) {
...
TEST(QueryIRTest, BuildsCreateMutatingPattern) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, KeepsBoundCreateEndpointsOutOfCreateCommands) {
...
TEST(QueryIRTest, BuildsUpdatingOnlyCreateWithPassthroughHorizon) {
  ...
  const std::string printed = ir::QueryIRToString(*query_ir);
  EXPECT_NE(printed.find("passthrough"), std::string::npos);
  EXPECT_NE(printed.find("clause: CREATE (n)"), std::string::npos);
}

TEST(QueryIRTest, BuildsUpdatingWithStarAfterCreate) {
  auto statement = ParseOrFail("MATCH () CREATE () WITH * CREATE ()");
  ASSERT_TRUE(statement);
  EXPECT_NO_THROW((void)ir::CreateQueryIR(*statement));
}

TEST(QueryIRTest, BuildsPatternComprehensionWithInlineNodeLabel) {
  auto statement =
...
TEST(QueryIRTest, BuildsSetDeleteAndRemoveMutatingPatterns) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, BuildsDetachDeleteMutatingPattern) {
...
TEST(QueryIRTest, IsolatesMergeInPassthroughSegment) {
  auto statement = ParseOrFail(
      "MATCH (m) MERGE (n) ON CREATE SET n.created = true RETURN n");
  ASSERT_TRUE(statement);

  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::SingleQueryIR &match_segment = query_ir->RequireSingle();
  ...
  const ir::SingleQueryIR &merge_segment = *match_segment.tail;
  ...
  const ir::SingleQueryIR &return_segment = *merge_segment.tail;
...
TEST(QueryIRTest, BuildsRelationshipMergeMatchGraph) {
  auto statement = ParseOrFail(
...
TEST(QueryIRTest, PreservesInputsAcrossConsecutiveMergeSegments) {
  auto statement = ParseOrFail(
  ...
  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::SingleQueryIR *segment = &query_ir->RequireSingle();
...
TEST(QueryIRTest, PreservesInputsAcrossUnwindSegments) {
  auto statement = ParseOrFail(
  ...
  std::unique_ptr<ir::QueryIR> query_ir = ir::CreateQueryIR(*statement);
  const ir::SingleQueryIR &unwind = *query_ir->RequireSingle().tail;
  EXPECT_TRUE(Contains(unwind.query_graph.argument_ids, "a"));
  ASSERT_TRUE(unwind.tail);
  const ir::SingleQueryIR &create = *unwind.tail;
  EXPECT_TRUE(Contains(create.query_graph.argument_ids, "a"));
  EXPECT_TRUE(Contains(create.query_graph.argument_ids, "i"));
}

TEST(QueryIRTest, SingleQueryIRIsMoveOnly) {
  EXPECT_FALSE(std::is_copy_constructible_v<ir::SingleQueryIR>);
  EXPECT_FALSE(std::is_copy_assignable_v<ir::SingleQueryIR>);
  EXPECT_TRUE(std::is_move_constructible_v<ir::SingleQueryIR>);
  EXPECT_TRUE(std::is_move_assignable_v<ir::SingleQueryIR>);
}
